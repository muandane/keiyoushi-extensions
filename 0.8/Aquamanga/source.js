...
var _Sources = (() => {
  ...
  // src/Aquamanga/Aquamanga.ts
  var Aquamanga_exports = {};
  __export(Aquamanga_exports, {
    Aquamanga: () => Aquamanga,
    AquamangaInfo: () => AquamangaInfo
  });
  var import_types = __toESM(require_lib());

  // src/Aquamanga/AquamangaParser.ts
  ...
  // src/Aquamanga/AquamangaSettings.ts
  ...
  // src/Aquamanga/Aquamanga.ts
  var AQUAMANGA_DOMAIN = "https://aquareader.net";
  var AquamangaInfo = {
    version: "1.0.0",
    name: "Aqua Manga",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Aqua Manga",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: AQUAMANGA_DOMAIN,
  ...
  var Aquamanga = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${AQUAMANGA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${AQUAMANGA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${AQUAMANGA_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${AQUAMANGA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${AQUAMANGA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Aquamanga_exports);
})();
