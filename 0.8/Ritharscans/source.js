...
var _Sources = (() => {
  ...
  // src/Ritharscans/Ritharscans.ts
  var Ritharscans_exports = {};
  __export(Ritharscans_exports, {
    Ritharscans: () => Ritharscans,
    RitharscansInfo: () => RitharscansInfo
  });
  var import_types = __toESM(require_lib());

  // src/Ritharscans/RitharscansParser.ts
  ...
  // src/Ritharscans/RitharscansSettings.ts
  ...
  // src/Ritharscans/Ritharscans.ts
  var RITHARSCANS_DOMAIN = "https://ritharscans.com";
  var RitharscansInfo = {
    version: "1.0.0",
    name: "RitharScans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from RitharScans",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: RITHARSCANS_DOMAIN,
  ...
  var Ritharscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${RITHARSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${RITHARSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${RITHARSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${RITHARSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${RITHARSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Ritharscans_exports);
})();
