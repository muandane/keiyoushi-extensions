...
var _Sources = (() => {
  ...
  // src/Resetscans/ResetscansParser.ts
  ...
  var parseChapters = ($, mangaId) => {
    ...
    for (let i = 0; i < chapterElements.length; i++) {
      ...
      const title = $("ul li .title, ul li .name, ul li h3", element).text().trim() || `Chapter ${i + 1}`;
      const chapNum = i + 1;
  ...
  // src/Resetscans/ResetscansSettings.ts
  ...
  // src/Resetscans/Resetscans.ts
  var RESETSCANS_DOMAIN = "https://reset-scans.org";
  var ResetscansInfo = {
    version: "1.0.0",
    name: "Reset Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Reset Scans",
  ...
  var Resetscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${RESETSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${RESETSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${RESETSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${RESETSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${RESETSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
