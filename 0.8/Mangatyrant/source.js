...
var _Sources = (() => {
  ...
  // src/Mangatyrant/Mangatyrant.ts
  var Mangatyrant_exports = {};
  __export(Mangatyrant_exports, {
    Mangatyrant: () => Mangatyrant,
    MangatyrantInfo: () => MangatyrantInfo
  });
  var import_types = __toESM(require_lib());

  // src/Mangatyrant/MangatyrantParser.ts
  ...
  // src/Mangatyrant/MangatyrantSettings.ts
  ...
  // src/Mangatyrant/Mangatyrant.ts
  var MANGATYRANT_DOMAIN = "https://mangatyrant.com";
  var MangatyrantInfo = {
    version: "1.0.0",
    name: "MangaTyrant",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from MangaTyrant",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: MANGATYRANT_DOMAIN,
  ...
  var Mangatyrant = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MANGATYRANT_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANGATYRANT_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MANGATYRANT_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGATYRANT_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGATYRANT_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Mangatyrant_exports);
})();
