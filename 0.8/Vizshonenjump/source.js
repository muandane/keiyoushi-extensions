...
var _Sources = (() => {
  ...
  // src/Vizshonenjump/Vizshonenjump.ts
  var Vizshonenjump_exports = {};
  __export(Vizshonenjump_exports, {
    Vizshonenjump: () => Vizshonenjump,
    VizshonenjumpInfo: () => VizshonenjumpInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Vizshonenjump/VizshonenjumpParser.ts
  ...
  // src/Vizshonenjump/VizshonenjumpSettings.ts
  ...
  // src/Vizshonenjump/Vizshonenjump.ts
  var VIZSHONENJUMP_DOMAIN = "https://www.viz.com";
  var VizshonenjumpInfo = {
    version: "1.0.0",
    name: "VIZ Shonen Jump",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from VIZ Shonen Jump",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: VIZSHONENJUMP_DOMAIN,
  ...
  var Vizshonenjump = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${VIZSHONENJUMP_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${VIZSHONENJUMP_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${VIZSHONENJUMP_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${VIZSHONENJUMP_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${VIZSHONENJUMP_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Vizshonenjump_exports);
})();
