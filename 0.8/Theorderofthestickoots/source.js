...
var _Sources = (() => {
  ...
  // src/Theorderofthestickoots/Theorderofthestickoots.ts
  var Theorderofthestickoots_exports = {};
  __export(Theorderofthestickoots_exports, {
    Theorderofthestickoots: () => Theorderofthestickoots,
    TheorderofthestickootsInfo: () => TheorderofthestickootsInfo
  });
  var import_types = __toESM(require_lib());

  // src/Theorderofthestickoots/TheorderofthestickootsParser.ts
  ...
  // src/Theorderofthestickoots/TheorderofthestickootsSettings.ts
  ...
  // src/Theorderofthestickoots/Theorderofthestickoots.ts
  var THEORDEROFTHESTICKOOTS_DOMAIN = "https://www.giantitp.com";
  var TheorderofthestickootsInfo = {
    version: "1.0.0",
    name: "The Order Of The Stick (OOTS)",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from The Order Of The Stick (OOTS)",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: THEORDEROFTHESTICKOOTS_DOMAIN,
  ...
  var Theorderofthestickoots = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${THEORDEROFTHESTICKOOTS_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${THEORDEROFTHESTICKOOTS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${THEORDEROFTHESTICKOOTS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${THEORDEROFTHESTICKOOTS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: THEORDEROFTHESTICKOOTS_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${THEORDEROFTHESTICKOOTS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${THEORDEROFTHESTICKOOTS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Theorderofthestickoots_exports);
})();
