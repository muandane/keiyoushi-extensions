...
var _Sources = (() => {
  ...
  // src/Roliascan/Roliascan.ts
  var Roliascan_exports = {};
  __export(Roliascan_exports, {
    Roliascan: () => Roliascan,
    RoliascanInfo: () => RoliascanInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Roliascan/RoliascanParser.ts
  ...
  // src/Roliascan/RoliascanSettings.ts
  ...
  // src/Roliascan/Roliascan.ts
  var ROLIASCAN_DOMAIN = "https://roliascan.com";
  var RoliascanInfo = {
    version: "1.0.0",
    name: "Rolia Scan",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Rolia Scan",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: ROLIASCAN_DOMAIN,
  ...
  var Roliascan = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${ROLIASCAN_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${ROLIASCAN_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${ROLIASCAN_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${ROLIASCAN_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${ROLIASCAN_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Roliascan_exports);
})();
