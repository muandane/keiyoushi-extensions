...
var _Sources = (() => {
  ...
  // src/Razure/Razure.ts
  var Razure_exports = {};
  __export(Razure_exports, {
    Razure: () => Razure,
    RazureInfo: () => RazureInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Razure/RazureParser.ts
  ...
  // src/Razure/Razure.ts
  var RAZURE_DOMAIN = "https://razure.org";
  var RazureInfo = {
    version: "1.0.0",
    name: "Razure",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Razure",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: RAZURE_DOMAIN,
  ...
  var Razure = class {
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${RAZURE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${RAZURE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Razure_exports);
})();
