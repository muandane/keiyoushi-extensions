...
var _Sources = (() => {
  ...
  var Rainofsnow_exports = {};
  __export(Rainofsnow_exports, {
    Rainofsnow: () => Rainofsnow,
    RainofsnowInfo: () => RainofsnowInfo
  });
  ...
  // src/Rainofsnow/RainofsnowSettings.ts
  ...
  // src/Rainofsnow/Rainofsnow.ts
  var RAINOFSNOW_DOMAIN = "https://rainofsnow.com";
  var RainofsnowInfo = {
    version: "1.0.0",
    name: "Rain Of Snow",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Rain Of Snow",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: RAINOFSNOW_DOMAIN,
  ...
  var Rainofsnow = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${RAINOFSNOW_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${RAINOFSNOW_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${RAINOFSNOW_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${RAINOFSNOW_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${RAINOFSNOW_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Rainofsnow_exports);
})();
