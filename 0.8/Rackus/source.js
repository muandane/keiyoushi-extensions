...
var _Sources = (() => {
  ...
  // src/Rackus/Rackus.ts
  var Rackus_exports = {};
  __export(Rackus_exports, {
    Rackus: () => Rackus,
    RackusInfo: () => RackusInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Rackus/RackusParser.ts
  ...
  // src/Rackus/Rackus.ts
  var RACKUS_DOMAIN = "https://rackusreads.com";
  var RackusInfo = {
    version: "1.0.0",
    name: "Rackus",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Rackus",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: RACKUS_DOMAIN,
  ...
  var Rackus = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${RACKUS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${RACKUS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${RACKUS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${RACKUS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Rackus_exports);
})();
