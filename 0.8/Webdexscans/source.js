...
var _Sources = (() => {
  ...
  // src/Webdexscans/Webdexscans.ts
  var Webdexscans_exports = {};
  __export(Webdexscans_exports, {
    Webdexscans: () => Webdexscans,
    WebdexscansInfo: () => WebdexscansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Webdexscans/WebdexscansParser.ts
  ...
  var parseHomeSections = ($, sectionCallback) => {
    ...
    for (const element of mangaElements.slice(0, 20)) {
      const title = $('[class*="manga"] .title, [class*="manga"] .name, [class*="manga"] h3', element).first().text().trim();
  ...
  // src/Webdexscans/WebdexscansSettings.ts
  ...
  // src/Webdexscans/Webdexscans.ts
  var WEBDEXSCANS_DOMAIN = "https://webdexscans.com";
  var WebdexscansInfo = {
    version: "1.0.0",
    name: "Webdex Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Webdex Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: WEBDEXSCANS_DOMAIN,
  ...
  var Webdexscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${WEBDEXSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${WEBDEXSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${WEBDEXSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${WEBDEXSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${WEBDEXSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Webdexscans_exports);
})();
