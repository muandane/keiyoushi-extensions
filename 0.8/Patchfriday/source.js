...
var _Sources = (() => {
  ...
  // src/Patchfriday/Patchfriday.ts
  var Patchfriday_exports = {};
  __export(Patchfriday_exports, {
    Patchfriday: () => Patchfriday,
    PatchfridayInfo: () => PatchfridayInfo
  });
  var import_types = __toESM(require_lib());

  // src/Patchfriday/PatchfridayParser.ts
  ...
  // src/Patchfriday/PatchfridaySettings.ts
  ...
  // src/Patchfriday/Patchfriday.ts
  var PATCHFRIDAY_DOMAIN = "https://patchfriday.com";
  var PatchfridayInfo = {
    version: "1.0.0",
    name: "Patch Friday",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Patch Friday",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: PATCHFRIDAY_DOMAIN,
  ...
  var Patchfriday = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${PATCHFRIDAY_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${PATCHFRIDAY_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${PATCHFRIDAY_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${PATCHFRIDAY_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${PATCHFRIDAY_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Patchfriday_exports);
})();
