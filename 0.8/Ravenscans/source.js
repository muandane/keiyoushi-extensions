...
var _Sources = (() => {
  ...
  // src/Ravenscans/Ravenscans.ts
  var Ravenscans_exports = {};
  __export(Ravenscans_exports, {
    Ravenscans: () => Ravenscans,
    RavenscansInfo: () => RavenscansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Ravenscans/RavenscansParser.ts
  ...
  // src/Ravenscans/RavenscansSettings.ts
  ...
  // src/Ravenscans/Ravenscans.ts
  var RAVENSCANS_DOMAIN = "https://ravenscans.com";
  var RavenscansInfo = {
    version: "1.0.0",
    name: "Raven Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Raven Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: RAVENSCANS_DOMAIN,
  ...
  var Ravenscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${RAVENSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${RAVENSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${RAVENSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${RAVENSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${RAVENSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Ravenscans_exports);
})();
