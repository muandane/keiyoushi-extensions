...
var _Sources = (() => {
  ...
  // src/Mangaplex/Mangaplex.ts
  var Mangaplex_exports = {};
  __export(Mangaplex_exports, {
    Mangaplex: () => Mangaplex,
    MangaplexInfo: () => MangaplexInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Mangaplex/MangaplexParser.ts
  ...
  // src/Mangaplex/MangaplexSettings.ts
  ...
  // src/Mangaplex/Mangaplex.ts
  var MANGAPLEX_DOMAIN = "https://mangaplex.com";
  var MangaplexInfo = {
    version: "1.0.0",
    name: "MangaPlex",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from MangaPlex",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MANGAPLEX_DOMAIN,
  ...
  var Mangaplex = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MANGAPLEX_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANGAPLEX_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MANGAPLEX_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGAPLEX_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGAPLEX_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Mangaplex_exports);
})();
