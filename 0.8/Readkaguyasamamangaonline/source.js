...
var _Sources = (() => {
  ...
  // src/Readkaguyasamamangaonline/Readkaguyasamamangaonline.ts
  var Readkaguyasamamangaonline_exports = {};
  __export(Readkaguyasamamangaonline_exports, {
    Readkaguyasamamangaonline: () => Readkaguyasamamangaonline,
    ReadkaguyasamamangaonlineInfo: () => ReadkaguyasamamangaonlineInfo
  });
  var import_types = __toESM(require_lib());

  // src/Readkaguyasamamangaonline/ReadkaguyasamamangaonlineParser.ts
  ...
  // src/Readkaguyasamamangaonline/ReadkaguyasamamangaonlineSettings.ts
  ...
  // src/Readkaguyasamamangaonline/Readkaguyasamamangaonline.ts
  var READKAGUYASAMAMANGAONLINE_DOMAIN = "https://ww4.readkaguyasama.com";
  var ReadkaguyasamamangaonlineInfo = {
    version: "1.0.0",
    name: "Read Kaguya-sama Manga Online",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Read Kaguya-sama Manga Online",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: READKAGUYASAMAMANGAONLINE_DOMAIN,
  ...
  var Readkaguyasamamangaonline = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${READKAGUYASAMAMANGAONLINE_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${READKAGUYASAMAMANGAONLINE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${READKAGUYASAMAMANGAONLINE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${READKAGUYASAMAMANGAONLINE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: READKAGUYASAMAMANGAONLINE_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${READKAGUYASAMAMANGAONLINE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${READKAGUYASAMAMANGAONLINE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Readkaguyasamamangaonline_exports);
})();
