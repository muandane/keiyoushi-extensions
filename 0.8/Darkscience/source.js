...
var _Sources = (() => {
  ...
  // src/Darkscience/Darkscience.ts
  var Darkscience_exports = {};
  __export(Darkscience_exports, {
    Darkscience: () => Darkscience,
    DarkscienceInfo: () => DarkscienceInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Darkscience/DarkscienceParser.ts
  ...
  // src/Darkscience/DarkscienceSettings.ts
  ...
  // src/Darkscience/Darkscience.ts
  var DARKSCIENCE_DOMAIN = "https://dresdencodak.com";
  var DarkscienceInfo = {
    version: "1.0.0",
    name: "Dark Science",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Dark Science",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: DARKSCIENCE_DOMAIN,
  ...
  var Darkscience = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${DARKSCIENCE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${DARKSCIENCE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${DARKSCIENCE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${DARKSCIENCE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${DARKSCIENCE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Darkscience_exports);
})();
