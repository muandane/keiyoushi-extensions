...
var _Sources = (() => {
  ...
  // src/Readnarutoborutosamurai8mangaonline/Readnarutoborutosamurai8mangaonline.ts
  var Readnarutoborutosamurai8mangaonline_exports = {};
  __export(Readnarutoborutosamurai8mangaonline_exports, {
    Readnarutoborutosamurai8mangaonline: () => Readnarutoborutosamurai8mangaonline,
    Readnarutoborutosamurai8mangaonlineInfo: () => Readnarutoborutosamurai8mangaonlineInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Readnarutoborutosamurai8mangaonline/Readnarutoborutosamurai8mangaonlineParser.ts
  ...
  // src/Readnarutoborutosamurai8mangaonline/Readnarutoborutosamurai8mangaonlineSettings.ts
  ...
  // src/Readnarutoborutosamurai8mangaonline/Readnarutoborutosamurai8mangaonline.ts
  var READNARUTOBORUTOSAMURAI8MANGAONLINE_DOMAIN = "https://ww11.readnaruto.com";
  var Readnarutoborutosamurai8mangaonlineInfo = {
    version: "1.0.0",
    name: "Read Naruto Boruto Samurai 8 Manga Online",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Read Naruto Boruto Samurai 8 Manga Online",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: READNARUTOBORUTOSAMURAI8MANGAONLINE_DOMAIN,
  ...
  var Readnarutoborutosamurai8mangaonline = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${READNARUTOBORUTOSAMURAI8MANGAONLINE_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${READNARUTOBORUTOSAMURAI8MANGAONLINE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${READNARUTOBORUTOSAMURAI8MANGAONLINE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${READNARUTOBORUTOSAMURAI8MANGAONLINE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: READNARUTOBORUTOSAMURAI8MANGAONLINE_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${READNARUTOBORUTOSAMURAI8MANGAONLINE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${READNARUTOBORUTOSAMURAI8MANGAONLINE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Readnarutoborutosamurai8mangaonline_exports);
})();
