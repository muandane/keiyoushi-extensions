...
var _Sources = (() => {
  ...
  // src/Madarascans/Madarascans.ts
  var Madarascans_exports = {};
  __export(Madarascans_exports, {
    Madarascans: () => Madarascans,
    MadarascansInfo: () => MadarascansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Madarascans/MadarascansParser.ts
  ...
  var parseChapters = ($, mangaId) => {
    const chapters = [];
    const chapterElements = $(".chapter-list").toArray();
    for (let i = 0; i < chapterElements.length; i++) {
      const element = chapterElements[i];
      const chapterId = $(".chapter-list a", element).attr("href")?.replace(/\//g, "").split("/").filter(Boolean).pop() || `chapter-${i}`;
      const title = $(".chapter-list a", element).text().trim() || `Chapter ${i + 1}`;
      const chapNum = i + 1;
  ...
  // src/Madarascans/MadarascansSettings.ts
  ...
  // src/Madarascans/Madarascans.ts
  var MADARASCANS_DOMAIN = "https://madarascans.com";
  var MadarascansInfo = {
    version: "1.0.0",
    name: "Madara Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Madara Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MADARASCANS_DOMAIN,
  ...
  var Madarascans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MADARASCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MADARASCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MADARASCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MADARASCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MADARASCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Madarascans_exports);
})();
