...
var _Sources = (() => {
  ...
  // src/Vgperson/Vgperson.ts
  var Vgperson_exports = {};
  __export(Vgperson_exports, {
    Vgperson: () => Vgperson,
    VgpersonInfo: () => VgpersonInfo
  });
  var import_types = __toESM(require_lib());

  // src/Vgperson/VgpersonParser.ts
  ...
  // src/Vgperson/VgpersonSettings.ts
  ...
  // src/Vgperson/Vgperson.ts
  var VGPERSON_DOMAIN = "https://vgperson.com";
  var VgpersonInfo = {
    version: "1.0.0",
    name: "vgperson",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from vgperson",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: VGPERSON_DOMAIN,
  ...
  var Vgperson = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${VGPERSON_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${VGPERSON_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${VGPERSON_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${VGPERSON_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Vgperson_exports);
})();
