...
var _Sources = (() => {
  ...
  // src/Thelibraryofohara/Thelibraryofohara.ts
  var Thelibraryofohara_exports = {};
  __export(Thelibraryofohara_exports, {
    Thelibraryofohara: () => Thelibraryofohara,
    ThelibraryofoharaInfo: () => ThelibraryofoharaInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Thelibraryofohara/ThelibraryofoharaParser.ts
  ...
  var parseChapters = ($, mangaId) => {
    ...
    for (let i = 0; i < chapterElements.length; i++) {
      ...
      const title = $('[class*="chapter"] h2', element).text().trim() || `Chapter ${i + 1}`;
      const chapNum = i + 1;
  ...
  var parseHomeSections = ($, sectionCallback) => {
    const latestItems = [];
    const mangaElements = $(".post").toArray();
    for (const element of mangaElements.slice(0, 20)) {
      const title = $(".post h2", element).first().text().trim();
      const image = $(".post img", element).first().attr("src") || $(".post img", element).first().attr("data-src") || "";
      const link = $(".post a", element).first().attr("href") || "";
  ...
  var parseViewMore = ($, homepageSectionId, metadata) => {
    ...
    for (const element of mangaElements) {
      const title = $(".post h2", element).first().text().trim();
      const image = $(".post img", element).first().attr("src") || "";
      const link = $(".post a", element).first().attr("href") || "";
      const mangaId = link.split("/").filter(Boolean).pop() || "";
      if (title && mangaId) {
  ...
  // src/Thelibraryofohara/ThelibraryofoharaSettings.ts
  ...
  // src/Thelibraryofohara/Thelibraryofohara.ts
  var THELIBRARYOFOHARA_DOMAIN = "https://thelibraryofohara.com";
  var ThelibraryofoharaInfo = {
    version: "1.0.0",
    name: "The Library of Ohara",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from The Library of Ohara",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: THELIBRARYOFOHARA_DOMAIN,
  ...
  var Thelibraryofohara = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${THELIBRARYOFOHARA_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${THELIBRARYOFOHARA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${THELIBRARYOFOHARA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${THELIBRARYOFOHARA_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${THELIBRARYOFOHARA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${THELIBRARYOFOHARA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Thelibraryofohara_exports);
})();
