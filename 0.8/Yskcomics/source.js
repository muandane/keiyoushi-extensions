...
var _Sources = (() => {
  ...
  // src/Yskcomics/Yskcomics.ts
  var Yskcomics_exports = {};
  __export(Yskcomics_exports, {
    Yskcomics: () => Yskcomics,
    YskcomicsInfo: () => YskcomicsInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Yskcomics/YskcomicsParser.ts
  ...
  // src/Yskcomics/YskcomicsSettings.ts
  ...
  // src/Yskcomics/Yskcomics.ts
  var YSKCOMICS_DOMAIN = "https://www.ysk-comics.com";
  var YskcomicsInfo = {
    version: "1.0.0",
    name: "YSK Comics",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from YSK Comics",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: YSKCOMICS_DOMAIN,
  ...
  var Yskcomics = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${YSKCOMICS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${YSKCOMICS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${YSKCOMICS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${YSKCOMICS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${YSKCOMICS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Yskcomics_exports);
})();
