...
var _Sources = (() => {
  ...
  // src/Elftoon/ElftoonSettings.ts
  ...
  // src/Elftoon/Elftoon.ts
  var ELFTOON_DOMAIN = "https://elftoon.com";
  var ElftoonInfo = {
    version: "1.0.0",
    name: "Elf Toon",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Elf Toon",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: ELFTOON_DOMAIN,
  ...
  var Elftoon = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${ELFTOON_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${ELFTOON_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${ELFTOON_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${ELFTOON_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
