...
var _Sources = (() => {
  ...
  // src/Aryascans/Aryascans.ts
  var Aryascans_exports = {};
  __export(Aryascans_exports, {
    Aryascans: () => Aryascans,
    AryascansInfo: () => AryascansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Aryascans/AryascansParser.ts
  ...
  // src/Aryascans/AryascansSettings.ts
  ...
  // src/Aryascans/Aryascans.ts
  var ARYASCANS_DOMAIN = "https://brainrotcomics.com";
  var AryascansInfo = {
    version: "1.0.0",
    name: "Arya Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Arya Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: ARYASCANS_DOMAIN,
  ...
  var Aryascans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${ARYASCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${ARYASCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${ARYASCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${ARYASCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${ARYASCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Aryascans_exports);
})();
