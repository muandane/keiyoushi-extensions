...
var _Sources = (() => {
  ...
  // src/Comicgrowl/Comicgrowl.ts
  var Comicgrowl_exports = {};
  __export(Comicgrowl_exports, {
    Comicgrowl: () => Comicgrowl,
    ComicgrowlInfo: () => ComicgrowlInfo
  });
  var import_types = __toESM(require_lib());

  // src/Comicgrowl/ComicgrowlParser.ts
  ...
  // src/Comicgrowl/ComicgrowlSettings.ts
  ...
  // src/Comicgrowl/Comicgrowl.ts
  var COMICGROWL_DOMAIN = "https://comic-growl.com";
  var ComicgrowlInfo = {
    version: "1.0.0",
    name: "Comic Growl",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Comic Growl",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: COMICGROWL_DOMAIN,
  ...
  var Comicgrowl = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${COMICGROWL_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${COMICGROWL_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${COMICGROWL_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${COMICGROWL_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${COMICGROWL_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Comicgrowl_exports);
})();
