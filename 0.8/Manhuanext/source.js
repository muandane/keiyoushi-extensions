...
var _Sources = (() => {
  ...
  // src/Manhuanext/Manhuanext.ts
  var Manhuanext_exports = {};
  __export(Manhuanext_exports, {
    Manhuanext: () => Manhuanext,
    ManhuanextInfo: () => ManhuanextInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Manhuanext/ManhuanextParser.ts
  ...
  // src/Manhuanext/ManhuanextSettings.ts
  ...
  // src/Manhuanext/Manhuanext.ts
  var MANHUANEXT_DOMAIN = "https://manhuanext.com";
  var ManhuanextInfo = {
    version: "1.0.0",
    name: "Manhuanext",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Manhuanext",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MANHUANEXT_DOMAIN,
  ...
  var Manhuanext = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MANHUANEXT_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANHUANEXT_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MANHUANEXT_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANHUANEXT_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANHUANEXT_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Manhuanext_exports);
})();
