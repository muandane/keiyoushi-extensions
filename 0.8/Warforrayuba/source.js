...
var _Sources = (() => {
  ...
  // src/Warforrayuba/Warforrayuba.ts
  var Warforrayuba_exports = {};
  __export(Warforrayuba_exports, {
    Warforrayuba: () => Warforrayuba,
    WarforrayubaInfo: () => WarforrayubaInfo
  });
  var import_types = __toESM(require_lib());

  // src/Warforrayuba/WarforrayubaParser.ts
  ...
  // src/Warforrayuba/WarforrayubaSettings.ts
  ...
  // src/Warforrayuba/Warforrayuba.ts
  var WARFORRAYUBA_DOMAIN = "https://xrabohrok.github.io";
  var WarforrayubaInfo = {
    version: "1.0.0",
    name: "War For Rayuba",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from War For Rayuba",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: WARFORRAYUBA_DOMAIN,
  ...
  var Warforrayuba = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${WARFORRAYUBA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${WARFORRAYUBA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${WARFORRAYUBA_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${WARFORRAYUBA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${WARFORRAYUBA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Warforrayuba_exports);
})();
