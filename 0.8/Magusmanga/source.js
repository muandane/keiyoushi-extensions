...
var _Sources = (() => {
  ...
  // src/Magusmanga/Magusmanga.ts
  var Magusmanga_exports = {};
  __export(Magusmanga_exports, {
    Magusmanga: () => Magusmanga,
    MagusmangaInfo: () => MagusmangaInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Magusmanga/MagusmangaParser.ts
  ...
  var parseHomeSections = ($, sectionCallback) => {
    ...
    for (const element of mangaElements.slice(0, 20)) {
      const title = $('[class*="manga"] .title', element).first().text().trim();
  ...
  // src/Magusmanga/MagusmangaSettings.ts
  ...
  // src/Magusmanga/Magusmanga.ts
  var MAGUSMANGA_DOMAIN = "https://magustoon.org";
  var MagusmangaInfo = {
    version: "1.0.0",
    name: "Magus Manga",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Magus Manga",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MAGUSMANGA_DOMAIN,
  ...
  var Magusmanga = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MAGUSMANGA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MAGUSMANGA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MAGUSMANGA_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MAGUSMANGA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MAGUSMANGA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Magusmanga_exports);
})();
