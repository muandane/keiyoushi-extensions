...
var _Sources = (() => {
  ...
  // src/Hivescans/Hivescans.ts
  var Hivescans_exports = {};
  __export(Hivescans_exports, {
    Hivescans: () => Hivescans,
    HivescansInfo: () => HivescansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Hivescans/HivescansParser.ts
  ...
  // src/Hivescans/HivescansSettings.ts
  ...
  // src/Hivescans/Hivescans.ts
  var HIVESCANS_DOMAIN = "https://hivetoons.org";
  var HivescansInfo = {
    version: "1.0.0",
    name: "Hive Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Hive Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: HIVESCANS_DOMAIN,
  ...
  var Hivescans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${HIVESCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${HIVESCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${HIVESCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${HIVESCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${HIVESCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Hivescans_exports);
})();
