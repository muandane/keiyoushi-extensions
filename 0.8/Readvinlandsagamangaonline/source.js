...
var _Sources = (() => {
  ...
  // src/Readvinlandsagamangaonline/Readvinlandsagamangaonline.ts
  var Readvinlandsagamangaonline_exports = {};
  __export(Readvinlandsagamangaonline_exports, {
    Readvinlandsagamangaonline: () => Readvinlandsagamangaonline,
    ReadvinlandsagamangaonlineInfo: () => ReadvinlandsagamangaonlineInfo
  });
  var import_types = __toESM(require_lib());

  // src/Readvinlandsagamangaonline/ReadvinlandsagamangaonlineParser.ts
  ...
  // src/Readvinlandsagamangaonline/ReadvinlandsagamangaonlineSettings.ts
  ...
  // src/Readvinlandsagamangaonline/Readvinlandsagamangaonline.ts
  var READVINLANDSAGAMANGAONLINE_DOMAIN = "https://ww5.readvinlandsaga.com";
  var ReadvinlandsagamangaonlineInfo = {
    version: "1.0.0",
    name: "Read Vinland Saga Manga Online",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Read Vinland Saga Manga Online",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: READVINLANDSAGAMANGAONLINE_DOMAIN,
  ...
  var Readvinlandsagamangaonline = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${READVINLANDSAGAMANGAONLINE_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${READVINLANDSAGAMANGAONLINE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${READVINLANDSAGAMANGAONLINE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${READVINLANDSAGAMANGAONLINE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: READVINLANDSAGAMANGAONLINE_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${READVINLANDSAGAMANGAONLINE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${READVINLANDSAGAMANGAONLINE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Readvinlandsagamangaonline_exports);
})();
