...
var _Sources = (() => {
  ...
  // src/Nyanukafe/Nyanukafe.ts
  var Nyanukafe_exports = {};
  __export(Nyanukafe_exports, {
    Nyanukafe: () => Nyanukafe,
    NyanukafeInfo: () => NyanukafeInfo
  });
  var import_types = __toESM(require_lib());

  // src/Nyanukafe/NyanukafeParser.ts
  ...
  // src/Nyanukafe/NyanukafeSettings.ts
  ...
  // src/Nyanukafe/Nyanukafe.ts
  var NYANUKAFE_DOMAIN = "https://nyanukafe.com";
  var NyanukafeInfo = {
    version: "1.0.0",
    name: "Nyanu Kafe",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Nyanu Kafe",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: NYANUKAFE_DOMAIN,
  ...
  var Nyanukafe = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${NYANUKAFE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${NYANUKAFE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${NYANUKAFE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${NYANUKAFE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${NYANUKAFE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Nyanukafe_exports);
})();
