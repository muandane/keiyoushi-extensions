...
var _Sources = (() => {
  ...
  // src/Darklegacycomics/Darklegacycomics.ts
  var Darklegacycomics_exports = {};
  __export(Darklegacycomics_exports, {
    Darklegacycomics: () => Darklegacycomics,
    DarklegacycomicsInfo: () => DarklegacycomicsInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Darklegacycomics/DarklegacycomicsParser.ts
  ...
  // src/Darklegacycomics/DarklegacycomicsSettings.ts
  ...
  // src/Darklegacycomics/Darklegacycomics.ts
  var DARKLEGACYCOMICS_DOMAIN = "https://www.darklegacycomics.com";
  var DarklegacycomicsInfo = {
    version: "1.0.0",
    name: "Dark Legacy Comics",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Dark Legacy Comics",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: DARKLEGACYCOMICS_DOMAIN,
  ...
  var Darklegacycomics = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${DARKLEGACYCOMICS_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${DARKLEGACYCOMICS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${DARKLEGACYCOMICS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${DARKLEGACYCOMICS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${DARKLEGACYCOMICS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${DARKLEGACYCOMICS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Darklegacycomics_exports);
})();
