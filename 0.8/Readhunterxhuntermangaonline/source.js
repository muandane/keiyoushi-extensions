...
var _Sources = (() => {
  ...
  // src/Readhunterxhuntermangaonline/Readhunterxhuntermangaonline.ts
  var Readhunterxhuntermangaonline_exports = {};
  __export(Readhunterxhuntermangaonline_exports, {
    Readhunterxhuntermangaonline: () => Readhunterxhuntermangaonline,
    ReadhunterxhuntermangaonlineInfo: () => ReadhunterxhuntermangaonlineInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Readhunterxhuntermangaonline/ReadhunterxhuntermangaonlineParser.ts
  ...
  // src/Readhunterxhuntermangaonline/ReadhunterxhuntermangaonlineSettings.ts
  ...
  // src/Readhunterxhuntermangaonline/Readhunterxhuntermangaonline.ts
  var READHUNTERXHUNTERMANGAONLINE_DOMAIN = "https://ww6.readhxh.com";
  var ReadhunterxhuntermangaonlineInfo = {
    version: "1.0.0",
    name: "Read Hunter x Hunter Manga Online",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Read Hunter x Hunter Manga Online",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: READHUNTERXHUNTERMANGAONLINE_DOMAIN,
  ...
  var Readhunterxhuntermangaonline = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${READHUNTERXHUNTERMANGAONLINE_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${READHUNTERXHUNTERMANGAONLINE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${READHUNTERXHUNTERMANGAONLINE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${READHUNTERXHUNTERMANGAONLINE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: READHUNTERXHUNTERMANGAONLINE_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${READHUNTERXHUNTERMANGAONLINE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${READHUNTERXHUNTERMANGAONLINE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Readhunterxhuntermangaonline_exports);
})();
