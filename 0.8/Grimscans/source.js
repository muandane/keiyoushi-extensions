...
var _Sources = (() => {
  ...
  // src/Grimscans/Grimscans.ts
  var Grimscans_exports = {};
  __export(Grimscans_exports, {
    Grimscans: () => Grimscans,
    GrimscansInfo: () => GrimscansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Grimscans/GrimscansParser.ts
  ...
  // src/Grimscans/GrimscansSettings.ts
  ...
  // src/Grimscans/Grimscans.ts
  var GRIMSCANS_DOMAIN = "https://grimscans.com";
  var GrimscansInfo = {
    version: "1.0.0",
    name: "Grim Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Grim Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: GRIMSCANS_DOMAIN,
  ...
  var Grimscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${GRIMSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${GRIMSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${GRIMSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${GRIMSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${GRIMSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Grimscans_exports);
})();
