...
var _Sources = (() => {
  ...
  // src/Asmodeusscans/Asmodeusscans.ts
  var Asmodeusscans_exports = {};
  __export(Asmodeusscans_exports, {
    Asmodeusscans: () => Asmodeusscans,
    AsmodeusscansInfo: () => AsmodeusscansInfo
  });
  var import_types = __toESM(require_lib());

  // src/Asmodeusscans/AsmodeusscansParser.ts
  ...
  // src/Asmodeusscans/AsmodeusscansSettings.ts
  ...
  // src/Asmodeusscans/Asmodeusscans.ts
  var ASMODEUSSCANS_DOMAIN = "https://asmotoon.com";
  var AsmodeusscansInfo = {
    version: "1.0.0",
    name: "Asmodeus Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Asmodeus Scans",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: ASMODEUSSCANS_DOMAIN,
  ...
  var Asmodeusscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${ASMODEUSSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${ASMODEUSSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${ASMODEUSSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${ASMODEUSSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${ASMODEUSSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Asmodeusscans_exports);
})();
