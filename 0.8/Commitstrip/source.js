...
var _Sources = (() => {
  ...
  // src/Commitstrip/Commitstrip.ts
  var Commitstrip_exports = {};
  __export(Commitstrip_exports, {
    Commitstrip: () => Commitstrip,
    CommitstripInfo: () => CommitstripInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Commitstrip/CommitstripParser.ts
  ...
  // src/Commitstrip/CommitstripSettings.ts
  ...
  // src/Commitstrip/Commitstrip.ts
  var COMMITSTRIP_DOMAIN = "https://www.commitstrip.com";
  var CommitstripInfo = {
    version: "1.0.0",
    name: "Commit Strip",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Commit Strip",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: COMMITSTRIP_DOMAIN,
  ...
  var Commitstrip = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${COMMITSTRIP_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${COMMITSTRIP_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${COMMITSTRIP_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${COMMITSTRIP_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${COMMITSTRIP_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Commitstrip_exports);
})();
