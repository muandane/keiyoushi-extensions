...
var _Sources = (() => {
  ...
  // src/Zinmangacom/Zinmangacom.ts
  var Zinmangacom_exports = {};
  __export(Zinmangacom_exports, {
    Zinmangacom: () => Zinmangacom,
    ZinmangacomInfo: () => ZinmangacomInfo
  });
  var import_types = __toESM(require_lib());

  // src/Zinmangacom/ZinmangacomParser.ts
  ...
  // src/Zinmangacom/ZinmangacomSettings.ts
  ...
  // src/Zinmangacom/Zinmangacom.ts
  var ZINMANGACOM_DOMAIN = "https://zin-manga.com";
  var ZinmangacomInfo = {
    version: "1.0.0",
    name: "Zin-Manga.com",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Zin-Manga.com",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: ZINMANGACOM_DOMAIN,
  ...
  var Zinmangacom = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${ZINMANGACOM_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${ZINMANGACOM_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${ZINMANGACOM_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${ZINMANGACOM_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${ZINMANGACOM_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Zinmangacom_exports);
})();
