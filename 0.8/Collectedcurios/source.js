...
var _Sources = (() => {
  ...
  // src/Collectedcurios/Collectedcurios.ts
  var Collectedcurios_exports = {};
  __export(Collectedcurios_exports, {
    Collectedcurios: () => Collectedcurios,
    CollectedcuriosInfo: () => CollectedcuriosInfo
  });
  var import_types = __toESM(require_lib());

  // src/Collectedcurios/CollectedcuriosParser.ts
  ...
  // src/Collectedcurios/CollectedcuriosSettings.ts
  ...
  // src/Collectedcurios/Collectedcurios.ts
  var COLLECTEDCURIOS_DOMAIN = "https://www.collectedcurios.com";
  var CollectedcuriosInfo = {
    version: "1.0.0",
    name: "Collected Curios",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Collected Curios",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: COLLECTEDCURIOS_DOMAIN,
  ...
  var Collectedcurios = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${COLLECTEDCURIOS_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${COLLECTEDCURIOS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${COLLECTEDCURIOS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${COLLECTEDCURIOS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${COLLECTEDCURIOS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${COLLECTEDCURIOS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Collectedcurios_exports);
})();
