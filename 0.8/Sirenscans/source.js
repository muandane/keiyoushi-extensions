...
var _Sources = (() => {
  ...
  // src/Sirenscans/Sirenscans.ts
  var Sirenscans_exports = {};
  __export(Sirenscans_exports, {
    Sirenscans: () => Sirenscans,
    SirenscansInfo: () => SirenscansInfo
  });
  var import_types = __toESM(require_lib());

  // src/Sirenscans/SirenscansParser.ts
  ...
  // src/Sirenscans/SirenscansSettings.ts
  ...
  // src/Sirenscans/Sirenscans.ts
  var SIRENSCANS_DOMAIN = "https://sirenscans.com";
  var SirenscansInfo = {
    version: "1.0.0",
    name: "Siren Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Siren Scans",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: SIRENSCANS_DOMAIN,
  ...
  var Sirenscans = class {
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${SIRENSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${SIRENSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${SIRENSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Sirenscans_exports);
})();
