...
var _Sources = (() => {
  ...
  // src/Rizzcomicunoriginal/Rizzcomicunoriginal.ts
  var Rizzcomicunoriginal_exports = {};
  __export(Rizzcomicunoriginal_exports, {
    Rizzcomicunoriginal: () => Rizzcomicunoriginal,
    RizzcomicunoriginalInfo: () => RizzcomicunoriginalInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Rizzcomicunoriginal/RizzcomicunoriginalParser.ts
  ...
  var parseHomeSections = ($, sectionCallback) => {
    const latestItems = [];
    const mangaElements = $('[class*="series"]').toArray();
    for (const element of mangaElements.slice(0, 20)) {
      const title = $('[class*="series"] a', element).first().text().trim();
      const image = $('[class*="series"] img', element).first().attr("src") || $('[class*="series"] img', element).first().attr("data-src") || "";
      const link = $('[class*="series"] a', element).first().attr("href") || "";
  ...
  var parseViewMore = ($, homepageSectionId, metadata) => {
    ...
    for (const element of mangaElements) {
      const title = $('[class*="series"] a', element).first().text().trim();
      const image = $('[class*="series"] img', element).first().attr("src") || "";
      const link = $('[class*="series"] a', element).first().attr("href") || "";
      const mangaId = link.split("/").filter(Boolean).pop() || "";
      if (title && mangaId) {
  ...
  // src/Rizzcomicunoriginal/RizzcomicunoriginalSettings.ts
  ...
  // src/Rizzcomicunoriginal/Rizzcomicunoriginal.ts
  var RIZZCOMICUNORIGINAL_DOMAIN = "https://rizzcomic.com";
  var RizzcomicunoriginalInfo = {
    version: "1.0.0",
    name: "Rizz Comic (unoriginal)",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Rizz Comic (unoriginal)",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: RIZZCOMICUNORIGINAL_DOMAIN,
  ...
  var Rizzcomicunoriginal = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${RIZZCOMICUNORIGINAL_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${RIZZCOMICUNORIGINAL_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${RIZZCOMICUNORIGINAL_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${RIZZCOMICUNORIGINAL_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: RIZZCOMICUNORIGINAL_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${RIZZCOMICUNORIGINAL_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${RIZZCOMICUNORIGINAL_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Rizzcomicunoriginal_exports);
})();
