...
var _Sources = (() => {
  ...
  // src/Ninemangaen/Ninemangaen.ts
  var Ninemangaen_exports = {};
  __export(Ninemangaen_exports, {
    Ninemangaen: () => Ninemangaen,
    NinemangaenInfo: () => NinemangaenInfo
  });
  var import_types = __toESM(require_lib());

  // src/Ninemangaen/NinemangaenParser.ts
  ...
  // src/Ninemangaen/NinemangaenSettings.ts
  ...
  // src/Ninemangaen/Ninemangaen.ts
  var NINEMANGAEN_DOMAIN = "https://www.ninemanga.com";
  var NinemangaenInfo = {
    version: "1.0.0",
    name: "NineMangaEn",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from NineMangaEn",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: NINEMANGAEN_DOMAIN,
  ...
  var Ninemangaen = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${NINEMANGAEN_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${NINEMANGAEN_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${NINEMANGAEN_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${NINEMANGAEN_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${NINEMANGAEN_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Ninemangaen_exports);
})();
