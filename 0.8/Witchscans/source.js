...
var _Sources = (() => {
  ...
  // src/Witchscans/Witchscans.ts
  var Witchscans_exports = {};
  __export(Witchscans_exports, {
    Witchscans: () => Witchscans,
    WitchscansInfo: () => WitchscansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Witchscans/WitchscansParser.ts
  ...
  // src/Witchscans/WitchscansSettings.ts
  ...
  // src/Witchscans/Witchscans.ts
  var WITCHSCANS_DOMAIN = "https://witchscans.com";
  var WitchscansInfo = {
    version: "1.0.0",
    name: "WitchScans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from WitchScans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: WITCHSCANS_DOMAIN,
  ...
  var Witchscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${WITCHSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${WITCHSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${WITCHSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${WITCHSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${WITCHSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Witchscans_exports);
})();
