...
var _Sources = (() => {
  ...
  // src/Likemanga/LikemangaParser.ts
  ...
  var parseHomeSections = ($, sectionCallback) => {
    ...
    for (const element of mangaElements.slice(0, 20)) {
      const title = $(".item h3", element).first().text().trim();
  ...
  // src/Likemanga/LikemangaSettings.ts
  ...
  // src/Likemanga/Likemanga.ts
  var LIKEMANGA_DOMAIN = "https://likemanga.ink";
  var LikemangaInfo = {
    version: "1.0.0",
    name: "LikeManga",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from LikeManga",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: LIKEMANGA_DOMAIN,
  ...
  var Likemanga = class {
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${LIKEMANGA_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${LIKEMANGA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${LIKEMANGA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
