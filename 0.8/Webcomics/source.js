...
var _Sources = (() => {
  ...
  // src/Webcomics/Webcomics.ts
  var Webcomics_exports = {};
  __export(Webcomics_exports, {
    Webcomics: () => Webcomics,
    WebcomicsInfo: () => WebcomicsInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Webcomics/WebcomicsParser.ts
  ...
  // src/Webcomics/WebcomicsSettings.ts
  ...
  // src/Webcomics/Webcomics.ts
  var WEBCOMICS_DOMAIN = "https://webcomicsapp.com";
  var WebcomicsInfo = {
    version: "1.0.0",
    name: "Webcomics",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Webcomics",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: WEBCOMICS_DOMAIN,
  ...
  var Webcomics = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${WEBCOMICS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${WEBCOMICS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${WEBCOMICS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${WEBCOMICS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${WEBCOMICS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Webcomics_exports);
})();
