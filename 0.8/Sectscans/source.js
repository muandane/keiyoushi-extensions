...
var _Sources = (() => {
  ...
  // src/Sectscans/Sectscans.ts
  var Sectscans_exports = {};
  __export(Sectscans_exports, {
    Sectscans: () => Sectscans,
    SectscansInfo: () => SectscansInfo
  });
  var import_types = __toESM(require_lib());

  // src/Sectscans/SectscansParser.ts
  ...
  // src/Sectscans/SectscansSettings.ts
  ...
  // src/Sectscans/Sectscans.ts
  var SECTSCANS_DOMAIN = "https://sectscans.com";
  var SectscansInfo = {
    version: "1.0.0",
    name: "SectScans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from SectScans",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: SECTSCANS_DOMAIN,
  ...
  var Sectscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${SECTSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${SECTSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${SECTSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${SECTSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${SECTSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Sectscans_exports);
})();
