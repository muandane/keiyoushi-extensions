...
var _Sources = (() => {
  ...
  // src/Deathtollscans/Deathtollscans.ts
  var Deathtollscans_exports = {};
  __export(Deathtollscans_exports, {
    Deathtollscans: () => Deathtollscans,
    DeathtollscansInfo: () => DeathtollscansInfo
  });
  var import_types = __toESM(require_lib());

  // src/Deathtollscans/DeathtollscansParser.ts
  ...
  // src/Deathtollscans/DeathtollscansSettings.ts
  ...
  // src/Deathtollscans/Deathtollscans.ts
  var DEATHTOLLSCANS_DOMAIN = "https://reader.deathtollscans.net";
  var DeathtollscansInfo = {
    version: "1.0.0",
    name: "Death Toll Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Death Toll Scans",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: DEATHTOLLSCANS_DOMAIN,
  ...
  var Deathtollscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${DEATHTOLLSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${DEATHTOLLSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${DEATHTOLLSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${DEATHTOLLSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${DEATHTOLLSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Deathtollscans_exports);
})();
