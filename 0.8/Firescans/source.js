...
var _Sources = (() => {
  ...
  // src/Firescans/Firescans.ts
  var Firescans_exports = {};
  __export(Firescans_exports, {
    Firescans: () => Firescans,
    FirescansInfo: () => FirescansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Firescans/FirescansParser.ts
  ...
  // src/Firescans/FirescansSettings.ts
  ...
  // src/Firescans/Firescans.ts
  var FIRESCANS_DOMAIN = "https://firescans.xyz";
  var FirescansInfo = {
    version: "1.0.0",
    name: "Firescans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Firescans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: FIRESCANS_DOMAIN,
  ...
  var Firescans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${FIRESCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${FIRESCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${FIRESCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${FIRESCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${FIRESCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Firescans_exports);
})();
