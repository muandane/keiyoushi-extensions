...
var _Sources = (() => {
  ...
  // src/Nyxscans/Nyxscans.ts
  var Nyxscans_exports = {};
  __export(Nyxscans_exports, {
    Nyxscans: () => Nyxscans,
    NyxscansInfo: () => NyxscansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Nyxscans/NyxscansParser.ts
  ...
  // src/Nyxscans/NyxscansSettings.ts
  ...
  // src/Nyxscans/Nyxscans.ts
  var NYXSCANS_DOMAIN = "https://nyxscans.com";
  var NyxscansInfo = {
    version: "1.0.0",
    name: "Nyx Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Nyx Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: NYXSCANS_DOMAIN,
  ...
  var Nyxscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${NYXSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${NYXSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${NYXSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${NYXSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Nyxscans_exports);
})();
