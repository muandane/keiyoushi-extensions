...
var _Sources = (() => {
  ...
  // src/Readsololevelingmangamanhwaonline/Readsololevelingmangamanhwaonline.ts
  var Readsololevelingmangamanhwaonline_exports = {};
  __export(Readsololevelingmangamanhwaonline_exports, {
    Readsololevelingmangamanhwaonline: () => Readsololevelingmangamanhwaonline,
    ReadsololevelingmangamanhwaonlineInfo: () => ReadsololevelingmangamanhwaonlineInfo
  });
  var import_types = __toESM(require_lib());

  // src/Readsololevelingmangamanhwaonline/ReadsololevelingmangamanhwaonlineParser.ts
  ...
  // src/Readsololevelingmangamanhwaonline/ReadsololevelingmangamanhwaonlineSettings.ts
  ...
  // src/Readsololevelingmangamanhwaonline/Readsololevelingmangamanhwaonline.ts
  var READSOLOLEVELINGMANGAMANHWAONLINE_DOMAIN = "https://ww3.readsololeveling.org";
  var ReadsololevelingmangamanhwaonlineInfo = {
    version: "1.0.0",
    name: "Read Solo Leveling Manga Manhwa Online",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Read Solo Leveling Manga Manhwa Online",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: READSOLOLEVELINGMANGAMANHWAONLINE_DOMAIN,
  ...
  var Readsololevelingmangamanhwaonline = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${READSOLOLEVELINGMANGAMANHWAONLINE_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${READSOLOLEVELINGMANGAMANHWAONLINE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${READSOLOLEVELINGMANGAMANHWAONLINE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${READSOLOLEVELINGMANGAMANHWAONLINE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: READSOLOLEVELINGMANGAMANHWAONLINE_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${READSOLOLEVELINGMANGAMANHWAONLINE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${READSOLOLEVELINGMANGAMANHWAONLINE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Readsololevelingmangamanhwaonline_exports);
})();
