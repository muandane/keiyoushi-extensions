...
var _Sources = (() => {
  ...
  // src/Mangatoonlimited/Mangatoonlimited.ts
  var Mangatoonlimited_exports = {};
  __export(Mangatoonlimited_exports, {
    Mangatoonlimited: () => Mangatoonlimited,
    MangatoonlimitedInfo: () => MangatoonlimitedInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Mangatoonlimited/MangatoonlimitedParser.ts
  ...
  var parseHomeSections = ($, sectionCallback) => {
    const latestItems = [];
    const mangaElements = $(".list-item").toArray();
    for (const element of mangaElements.slice(0, 20)) {
      const title = $(".list-item h2", element).first().text().trim();
      const image = $(".list-item img", element).first().attr("src") || $(".list-item img", element).first().attr("data-src") || "";
      const link = $(".list-item a", element).first().attr("href") || "";
  ...
  var parseViewMore = ($, homepageSectionId, metadata) => {
    ...
    for (const element of mangaElements) {
      const title = $(".list-item h2", element).first().text().trim();
      const image = $(".list-item img", element).first().attr("src") || "";
      const link = $(".list-item a", element).first().attr("href") || "";
      const mangaId = link.split("/").filter(Boolean).pop() || "";
      if (title && mangaId) {
  ...
  // src/Mangatoonlimited/MangatoonlimitedSettings.ts
  ...
  // src/Mangatoonlimited/Mangatoonlimited.ts
  var MANGATOONLIMITED_DOMAIN = "https://mangatoon.mobi";
  var MangatoonlimitedInfo = {
    version: "1.0.0",
    name: "MangaToon (Limited)",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from MangaToon (Limited)",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MANGATOONLIMITED_DOMAIN,
  ...
  var Mangatoonlimited = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${MANGATOONLIMITED_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${MANGATOONLIMITED_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANGATOONLIMITED_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MANGATOONLIMITED_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGATOONLIMITED_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGATOONLIMITED_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Mangatoonlimited_exports);
})();
