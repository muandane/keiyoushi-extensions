...
var _Sources = (() => {
  ...
  // src/Sanascans/Sanascans.ts
  var Sanascans_exports = {};
  __export(Sanascans_exports, {
    Sanascans: () => Sanascans,
    SanascansInfo: () => SanascansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Sanascans/SanascansParser.ts
  ...
  // src/Sanascans/SanascansSettings.ts
  ...
  // src/Sanascans/Sanascans.ts
  var SANASCANS_DOMAIN = "https://sanascans.com";
  var SanascansInfo = {
    version: "1.0.0",
    name: "Sana Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Sana Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: SANASCANS_DOMAIN,
  ...
  var Sanascans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${SANASCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${SANASCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${SANASCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${SANASCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${SANASCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Sanascans_exports);
})();
