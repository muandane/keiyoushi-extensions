...
var _Sources = (() => {
  ...
  // src/Projectsuki/ProjectsukiParser.ts
  ...
  // src/Projectsuki/ProjectsukiSettings.ts
  ...
  // src/Projectsuki/Projectsuki.ts
  var PROJECTSUKI_DOMAIN = "https://projectsuki.com";
  var ProjectsukiInfo = {
    version: "1.0.0",
    name: "Project Suki",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Project Suki",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: PROJECTSUKI_DOMAIN,
  ...
  var Projectsuki = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${PROJECTSUKI_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${PROJECTSUKI_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${PROJECTSUKI_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${PROJECTSUKI_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${PROJECTSUKI_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
