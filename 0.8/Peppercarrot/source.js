...
var _Sources = (() => {
  ...
  // src/Peppercarrot/PeppercarrotSettings.ts
  ...
  // src/Peppercarrot/Peppercarrot.ts
  var PEPPERCARROT_DOMAIN = "https://www.peppercarrot.com";
  var PeppercarrotInfo = {
    ...
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Pepper&Carrot",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: PEPPERCARROT_DOMAIN,
  ...
  var Peppercarrot = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${PEPPERCARROT_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${PEPPERCARROT_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${PEPPERCARROT_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${PEPPERCARROT_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${PEPPERCARROT_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
