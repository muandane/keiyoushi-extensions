...
var _Sources = (() => {
  ...
  // src/Casacomic/Casacomic.ts
  var Casacomic_exports = {};
  __export(Casacomic_exports, {
    Casacomic: () => Casacomic,
    CasacomicInfo: () => CasacomicInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Casacomic/CasacomicParser.ts
  ...
  // src/Casacomic/CasacomicSettings.ts
  ...
  // src/Casacomic/Casacomic.ts
  var CASACOMIC_DOMAIN = "https://casacomic.com";
  var CasacomicInfo = {
    version: "1.0.0",
    name: "Casa Comic",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Casa Comic",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: CASACOMIC_DOMAIN,
  ...
  var Casacomic = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${CASACOMIC_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${CASACOMIC_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${CASACOMIC_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${CASACOMIC_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${CASACOMIC_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Casacomic_exports);
})();
