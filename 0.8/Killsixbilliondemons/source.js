...
var _Sources = (() => {
  ...
  // src/Killsixbilliondemons/Killsixbilliondemons.ts
  var Killsixbilliondemons_exports = {};
  __export(Killsixbilliondemons_exports, {
    Killsixbilliondemons: () => Killsixbilliondemons,
    KillsixbilliondemonsInfo: () => KillsixbilliondemonsInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Killsixbilliondemons/KillsixbilliondemonsParser.ts
  ...
  var parseChapters = ($, mangaId) => {
    ...
    for (let i = 0; i < chapterElements.length; i++) {
      ...
      const chapterId = $("a", element).attr("href")?.replace(/\//g, "").split("/").filter(Boolean).pop() || `chapter-${i}`;
      const title = $('[class*="chapter"] .title, [class*="chapter"] .name, [class*="chapter"] h3', element).text().trim() || `Chapter ${i + 1}`;
      const chapNum = i + 1;
  ...
  var parseHomeSections = ($, sectionCallback) => {
    const latestItems = [];
    const mangaElements = $('[class*="comic"]').toArray();
    for (const element of mangaElements.slice(0, 20)) {
      const title = $('[class*="comic"] a', element).first().text().trim();
      const image = $('[class*="comic"] img', element).first().attr("src") || $('[class*="comic"] img', element).first().attr("data-src") || "";
      const link = $('[class*="comic"] a', element).first().attr("href") || "";
  ...
  var parseViewMore = ($, homepageSectionId, metadata) => {
    ...
    for (const element of mangaElements) {
      const title = $('[class*="comic"] a', element).first().text().trim();
      const image = $('[class*="comic"] img', element).first().attr("src") || "";
      const link = $('[class*="comic"] a', element).first().attr("href") || "";
      const mangaId = link.split("/").filter(Boolean).pop() || "";
      if (title && mangaId) {
  ...
  // src/Killsixbilliondemons/KillsixbilliondemonsSettings.ts
  ...
  // src/Killsixbilliondemons/Killsixbilliondemons.ts
  var KILLSIXBILLIONDEMONS_DOMAIN = "https://killsixbilliondemons.com";
  var KillsixbilliondemonsInfo = {
    version: "1.0.0",
    name: "KillSixBillionDemons",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from KillSixBillionDemons",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: KILLSIXBILLIONDEMONS_DOMAIN,
  ...
  var Killsixbilliondemons = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${KILLSIXBILLIONDEMONS_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${KILLSIXBILLIONDEMONS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${KILLSIXBILLIONDEMONS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${KILLSIXBILLIONDEMONS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: KILLSIXBILLIONDEMONS_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${KILLSIXBILLIONDEMONS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${KILLSIXBILLIONDEMONS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Killsixbilliondemons_exports);
})();
