...
var _Sources = (() => {
  ...
  // src/Comichubfree/Comichubfree.ts
  var Comichubfree_exports = {};
  __export(Comichubfree_exports, {
    Comichubfree: () => Comichubfree,
    ComichubfreeInfo: () => ComichubfreeInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Comichubfree/ComichubfreeParser.ts
  ...
  // src/Comichubfree/ComichubfreeSettings.ts
  ...
  // src/Comichubfree/Comichubfree.ts
  var COMICHUBFREE_DOMAIN = "https://comichubfree.com";
  var ComichubfreeInfo = {
    version: "1.0.0",
    name: "ComicHubFree",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from ComicHubFree",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: COMICHUBFREE_DOMAIN,
  ...
  var Comichubfree = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${COMICHUBFREE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${COMICHUBFREE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${COMICHUBFREE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${COMICHUBFREE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${COMICHUBFREE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Comichubfree_exports);
})();
