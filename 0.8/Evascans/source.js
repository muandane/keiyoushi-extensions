...
var _Sources = (() => {
  ...
  // src/Evascans/Evascans.ts
  var Evascans_exports = {};
  __export(Evascans_exports, {
    Evascans: () => Evascans,
    EvascansInfo: () => EvascansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Evascans/EvascansParser.ts
  ...
  // src/Evascans/EvascansSettings.ts
  ...
  // src/Evascans/Evascans.ts
  var EVASCANS_DOMAIN = "https://evascans.org";
  var EvascansInfo = {
    version: "1.0.0",
    name: "Eva Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Eva Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: EVASCANS_DOMAIN,
  ...
  var Evascans = class {
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${EVASCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${EVASCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Evascans_exports);
})();
