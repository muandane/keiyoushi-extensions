...
var _Sources = (() => {
  ...
  // src/Aeinscans/Aeinscans.ts
  var Aeinscans_exports = {};
  __export(Aeinscans_exports, {
    Aeinscans: () => Aeinscans,
    AeinscansInfo: () => AeinscansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Aeinscans/AeinscansParser.ts
  ...
  // src/Aeinscans/AeinscansSettings.ts
  ...
  // src/Aeinscans/Aeinscans.ts
  var AEINSCANS_DOMAIN = "https://aeinscans.com";
  var AeinscansInfo = {
    version: "1.0.0",
    name: "Aein Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Aein Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: AEINSCANS_DOMAIN,
  ...
  var Aeinscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${AEINSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${AEINSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${AEINSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${AEINSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${AEINSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Aeinscans_exports);
})();
