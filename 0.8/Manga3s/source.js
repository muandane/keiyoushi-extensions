...
var _Sources = (() => {
  ...
  // src/Manga3s/Manga3s.ts
  var Manga3s_exports = {};
  __export(Manga3s_exports, {
    Manga3s: () => Manga3s,
    Manga3sInfo: () => Manga3sInfo
  });
  var import_types = __toESM(require_lib());

  // src/Manga3s/Manga3sParser.ts
  ...
  // src/Manga3s/Manga3sSettings.ts
  ...
  // src/Manga3s/Manga3s.ts
  var MANGA3S_DOMAIN = "https://manga3s.com";
  var Manga3sInfo = {
    version: "1.0.0",
    name: "Manga3S",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Manga3S",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: MANGA3S_DOMAIN,
  ...
  var Manga3s = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MANGA3S_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANGA3S_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGA3S_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGA3S_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Manga3s_exports);
})();
