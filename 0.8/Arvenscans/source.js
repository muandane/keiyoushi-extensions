...
var _Sources = (() => {
  ...
  // src/Arvenscans/ArvenscansParser.ts
  ...
  // src/Arvenscans/ArvenscansSettings.ts
  ...
  // src/Arvenscans/Arvenscans.ts
  var ARVENSCANS_DOMAIN = "https://arvencomics.com";
  var ArvenscansInfo = {
    version: "1.0.0",
    name: "Arven Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Arven Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: ARVENSCANS_DOMAIN,
  ...
  var Arvenscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${ARVENSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${ARVENSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${ARVENSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${ARVENSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${ARVENSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
