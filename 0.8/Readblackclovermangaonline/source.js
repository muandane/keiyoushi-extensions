...
var _Sources = (() => {
  ...
  // src/Readblackclovermangaonline/Readblackclovermangaonline.ts
  var Readblackclovermangaonline_exports = {};
  __export(Readblackclovermangaonline_exports, {
    Readblackclovermangaonline: () => Readblackclovermangaonline,
    ReadblackclovermangaonlineInfo: () => ReadblackclovermangaonlineInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Readblackclovermangaonline/ReadblackclovermangaonlineParser.ts
  ...
  // src/Readblackclovermangaonline/ReadblackclovermangaonlineSettings.ts
  ...
  // src/Readblackclovermangaonline/Readblackclovermangaonline.ts
  var READBLACKCLOVERMANGAONLINE_DOMAIN = "https://ww10.readblackclover.com";
  var ReadblackclovermangaonlineInfo = {
    version: "1.0.0",
    name: "Read Black Clover Manga Online",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Read Black Clover Manga Online",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: READBLACKCLOVERMANGAONLINE_DOMAIN,
  ...
  var Readblackclovermangaonline = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${READBLACKCLOVERMANGAONLINE_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${READBLACKCLOVERMANGAONLINE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${READBLACKCLOVERMANGAONLINE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${READBLACKCLOVERMANGAONLINE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: READBLACKCLOVERMANGAONLINE_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${READBLACKCLOVERMANGAONLINE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${READBLACKCLOVERMANGAONLINE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Readblackclovermangaonline_exports);
})();
