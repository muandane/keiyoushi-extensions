...
var _Sources = (() => {
  ...
  // src/Existentialcomics/Existentialcomics.ts
  var Existentialcomics_exports = {};
  __export(Existentialcomics_exports, {
    Existentialcomics: () => Existentialcomics,
    ExistentialcomicsInfo: () => ExistentialcomicsInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Existentialcomics/ExistentialcomicsParser.ts
  ...
  // src/Existentialcomics/ExistentialcomicsSettings.ts
  ...
  // src/Existentialcomics/Existentialcomics.ts
  var EXISTENTIALCOMICS_DOMAIN = "https://existentialcomics.com";
  var ExistentialcomicsInfo = {
    version: "1.0.0",
    name: "Existential Comics",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Existential Comics",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: EXISTENTIALCOMICS_DOMAIN,
  ...
  var Existentialcomics = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${EXISTENTIALCOMICS_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${EXISTENTIALCOMICS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${EXISTENTIALCOMICS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${EXISTENTIALCOMICS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${EXISTENTIALCOMICS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${EXISTENTIALCOMICS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Existentialcomics_exports);
})();
