...
var _Sources = (() => {
  ...
  // src/Mangakiss/Mangakiss.ts
  var Mangakiss_exports = {};
  __export(Mangakiss_exports, {
    Mangakiss: () => Mangakiss,
    MangakissInfo: () => MangakissInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Mangakiss/MangakissParser.ts
  ...
  // src/Mangakiss/MangakissSettings.ts
  ...
  // src/Mangakiss/Mangakiss.ts
  var MANGAKISS_DOMAIN = "https://mangakiss.org";
  var MangakissInfo = {
    version: "1.0.0",
    name: "Manga Kiss",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Manga Kiss",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MANGAKISS_DOMAIN,
  ...
  var Mangakiss = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MANGAKISS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANGAKISS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MANGAKISS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGAKISS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGAKISS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Mangakiss_exports);
})();
