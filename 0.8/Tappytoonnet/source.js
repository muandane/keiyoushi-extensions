...
var _Sources = (() => {
  ...
  // src/Tappytoonnet/Tappytoonnet.ts
  var Tappytoonnet_exports = {};
  __export(Tappytoonnet_exports, {
    Tappytoonnet: () => Tappytoonnet,
    TappytoonnetInfo: () => TappytoonnetInfo
  });
  var import_types = __toESM(require_lib());

  // src/Tappytoonnet/TappytoonnetParser.ts
  ...
  // src/Tappytoonnet/TappytoonnetSettings.ts
  ...
  // src/Tappytoonnet/Tappytoonnet.ts
  var TAPPYTOONNET_DOMAIN = "https://tappytoon.net";
  var TappytoonnetInfo = {
    version: "1.0.0",
    name: "TappyToon.Net",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from TappyToon.Net",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: TAPPYTOONNET_DOMAIN,
  ...
  var Tappytoonnet = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${TAPPYTOONNET_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${TAPPYTOONNET_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${TAPPYTOONNET_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${TAPPYTOONNET_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${TAPPYTOONNET_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Tappytoonnet_exports);
})();
