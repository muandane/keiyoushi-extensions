...
var _Sources = (() => {
  ...
  // src/Manhuaaz/Manhuaaz.ts
  var Manhuaaz_exports = {};
  __export(Manhuaaz_exports, {
    Manhuaaz: () => Manhuaaz,
    ManhuaazInfo: () => ManhuaazInfo
  });
  var import_types = __toESM(require_lib());

  // src/Manhuaaz/ManhuaazParser.ts
  ...
  // src/Manhuaaz/ManhuaazSettings.ts
  ...
  // src/Manhuaaz/Manhuaaz.ts
  var MANHUAAZ_DOMAIN = "https://manhuaaz.com";
  var ManhuaazInfo = {
    version: "1.0.0",
    name: "Manhua AZ",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Manhua AZ",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: MANHUAAZ_DOMAIN,
  ...
  var Manhuaaz = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MANHUAAZ_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANHUAAZ_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANHUAAZ_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANHUAAZ_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Manhuaaz_exports);
})();
