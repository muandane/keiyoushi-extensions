...
var _Sources = (() => {
  ...
  // src/Webniichan/Webniichan.ts
  var Webniichan_exports = {};
  __export(Webniichan_exports, {
    Webniichan: () => Webniichan,
    WebniichanInfo: () => WebniichanInfo
  });
  var import_types = __toESM(require_lib());

  // src/Webniichan/WebniichanParser.ts
  ...
  // src/Webniichan/WebniichanSettings.ts
  ...
  // src/Webniichan/Webniichan.ts
  var WEBNIICHAN_DOMAIN = "https://webniichan.online";
  var WebniichanInfo = {
    version: "1.0.0",
    name: "Web Niichan",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Web Niichan",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: WEBNIICHAN_DOMAIN,
  ...
  var Webniichan = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${WEBNIICHAN_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${WEBNIICHAN_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${WEBNIICHAN_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${WEBNIICHAN_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${WEBNIICHAN_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Webniichan_exports);
})();
