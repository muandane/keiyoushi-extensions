...
var _Sources = (() => {
  ...
  // src/Webtoonscom/Webtoonscom.ts
  var Webtoonscom_exports = {};
  __export(Webtoonscom_exports, {
    Webtoonscom: () => Webtoonscom,
    WebtoonscomInfo: () => WebtoonscomInfo
  });
  var import_types = __toESM(require_lib());

  // src/Webtoonscom/WebtoonscomParser.ts
  ...
  // src/Webtoonscom/WebtoonscomSettings.ts
  ...
  // src/Webtoonscom/Webtoonscom.ts
  var WEBTOONSCOM_DOMAIN = "https://www.webtoons.com";
  var WebtoonscomInfo = {
    version: "1.0.0",
    name: "Webtoons.com",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Webtoons.com",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: WEBTOONSCOM_DOMAIN,
  ...
  var Webtoonscom = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${WEBTOONSCOM_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${WEBTOONSCOM_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${WEBTOONSCOM_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${WEBTOONSCOM_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${WEBTOONSCOM_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Webtoonscom_exports);
})();
