...
var _Sources = (() => {
  ...
  // src/Violetscans/VioletscansParser.ts
  ...
  // src/Violetscans/VioletscansSettings.ts
  ...
  // src/Violetscans/Violetscans.ts
  var VIOLETSCANS_DOMAIN = "https://violetscans.org";
  var VioletscansInfo = {
    version: "1.0.0",
    name: "Violet Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Violet Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: VIOLETSCANS_DOMAIN,
  ...
  var Violetscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${VIOLETSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${VIOLETSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${VIOLETSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${VIOLETSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${VIOLETSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
