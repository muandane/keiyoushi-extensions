...
var _Sources = (() => {
  ...
  // src/Webnovel/Webnovel.ts
  var Webnovel_exports = {};
  __export(Webnovel_exports, {
    Webnovel: () => Webnovel,
    WebnovelInfo: () => WebnovelInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Webnovel/WebnovelParser.ts
  ...
  // src/Webnovel/WebnovelSettings.ts
  ...
  // src/Webnovel/Webnovel.ts
  var WEBNOVEL_DOMAIN = "https://www.webnovel.com";
  var WebnovelInfo = {
    version: "1.0.0",
    name: "WebNovel",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from WebNovel",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: WEBNOVEL_DOMAIN,
  ...
  var Webnovel = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${WEBNOVEL_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${WEBNOVEL_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${WEBNOVEL_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${WEBNOVEL_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Webnovel_exports);
})();
