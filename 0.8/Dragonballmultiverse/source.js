...
var _Sources = (() => {
  ...
  // src/Dragonballmultiverse/Dragonballmultiverse.ts
  var Dragonballmultiverse_exports = {};
  __export(Dragonballmultiverse_exports, {
    Dragonballmultiverse: () => Dragonballmultiverse,
    DragonballmultiverseInfo: () => DragonballmultiverseInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Dragonballmultiverse/DragonballmultiverseParser.ts
  ...
  // src/Dragonballmultiverse/DragonballmultiverseSettings.ts
  ...
  // src/Dragonballmultiverse/Dragonballmultiverse.ts
  var DRAGONBALLMULTIVERSE_DOMAIN = "https://www.dragonball-multiverse.com";
  var DragonballmultiverseInfo = {
    version: "1.0.0",
    name: "Dragon Ball Multiverse",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Dragon Ball Multiverse",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: DRAGONBALLMULTIVERSE_DOMAIN,
  ...
  var Dragonballmultiverse = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${DRAGONBALLMULTIVERSE_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${DRAGONBALLMULTIVERSE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${DRAGONBALLMULTIVERSE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${DRAGONBALLMULTIVERSE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: DRAGONBALLMULTIVERSE_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${DRAGONBALLMULTIVERSE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${DRAGONBALLMULTIVERSE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Dragonballmultiverse_exports);
})();
