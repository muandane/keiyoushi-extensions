...
var _Sources = (() => {
  ...
  // src/Swordscomic/Swordscomic.ts
  var Swordscomic_exports = {};
  __export(Swordscomic_exports, {
    Swordscomic: () => Swordscomic,
    SwordscomicInfo: () => SwordscomicInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Swordscomic/SwordscomicParser.ts
  ...
  // src/Swordscomic/SwordscomicSettings.ts
  ...
  // src/Swordscomic/Swordscomic.ts
  var SWORDSCOMIC_DOMAIN = "https://swordscomic.com";
  var SwordscomicInfo = {
    version: "1.0.0",
    name: "Swords Comic",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Swords Comic",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: SWORDSCOMIC_DOMAIN,
  ...
  var Swordscomic = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${SWORDSCOMIC_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${SWORDSCOMIC_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${SWORDSCOMIC_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${SWORDSCOMIC_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${SWORDSCOMIC_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Swordscomic_exports);
})();
