...
var _Sources = (() => {
  ...
  // src/Toonchill/Toonchill.ts
  var Toonchill_exports = {};
  __export(Toonchill_exports, {
    Toonchill: () => Toonchill,
    ToonchillInfo: () => ToonchillInfo
  });
  var import_types = __toESM(require_lib());

  // src/Toonchill/ToonchillParser.ts
  ...
  // src/Toonchill/ToonchillSettings.ts
  ...
  // src/Toonchill/Toonchill.ts
  var TOONCHILL_DOMAIN = "https://toonchill.com";
  var ToonchillInfo = {
    version: "1.0.0",
    name: "ToonChill",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from ToonChill",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: TOONCHILL_DOMAIN,
  ...
  var Toonchill = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${TOONCHILL_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${TOONCHILL_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${TOONCHILL_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${TOONCHILL_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${TOONCHILL_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Toonchill_exports);
})();
