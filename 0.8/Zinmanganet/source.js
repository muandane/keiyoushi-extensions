...
var _Sources = (() => {
  ...
  // src/Zinmanganet/Zinmanganet.ts
  var Zinmanganet_exports = {};
  __export(Zinmanganet_exports, {
    Zinmanganet: () => Zinmanganet,
    ZinmanganetInfo: () => ZinmanganetInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Zinmanganet/ZinmanganetParser.ts
  ...
  // src/Zinmanganet/ZinmanganetSettings.ts
  ...
  // src/Zinmanganet/Zinmanganet.ts
  var ZINMANGANET_DOMAIN = "https://zinmanga.net";
  var ZinmanganetInfo = {
    version: "1.0.0",
    name: "Zinmanga.net",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Zinmanga.net",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: ZINMANGANET_DOMAIN,
  ...
  var Zinmanganet = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${ZINMANGANET_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${ZINMANGANET_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${ZINMANGANET_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${ZINMANGANET_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${ZINMANGANET_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Zinmanganet_exports);
})();
