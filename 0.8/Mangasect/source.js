...
var _Sources = (() => {
  ...
  // src/Mangasect/Mangasect.ts
  var Mangasect_exports = {};
  __export(Mangasect_exports, {
    Mangasect: () => Mangasect,
    MangasectInfo: () => MangasectInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Mangasect/MangasectParser.ts
  ...
  // src/Mangasect/MangasectSettings.ts
  ...
  // src/Mangasect/Mangasect.ts
  var MANGASECT_DOMAIN = "https://mangasect.net";
  var MangasectInfo = {
    version: "1.0.0",
    name: "Manga Sect",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Manga Sect",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MANGASECT_DOMAIN,
  ...
  var Mangasect = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MANGASECT_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANGASECT_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MANGASECT_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGASECT_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGASECT_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Mangasect_exports);
})();
