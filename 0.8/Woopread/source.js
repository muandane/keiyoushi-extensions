...
var _Sources = (() => {
  ...
  // src/Woopread/Woopread.ts
  var Woopread_exports = {};
  __export(Woopread_exports, {
    Woopread: () => Woopread,
    WoopreadInfo: () => WoopreadInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Woopread/WoopreadParser.ts
  ...
  var parseChapters = ($, mangaId) => {
    ...
    for (let i = 0; i < chapterElements.length; i++) {
      ...
      const title = $("ul li h4", element).text().trim() || `Chapter ${i + 1}`;
      const chapNum = i + 1;
  ...
  // src/Woopread/WoopreadSettings.ts
  ...
  // src/Woopread/Woopread.ts
  var WOOPREAD_DOMAIN = "https://woopread.com";
  var WoopreadInfo = {
    version: "1.0.0",
    name: "WoopRead",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from WoopRead",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: WOOPREAD_DOMAIN,
  ...
  var Woopread = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${WOOPREAD_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${WOOPREAD_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${WOOPREAD_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${WOOPREAD_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Woopread_exports);
})();
