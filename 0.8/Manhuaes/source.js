...
var _Sources = (() => {
  ...
  // src/Manhuaes/Manhuaes.ts
  var Manhuaes_exports = {};
  __export(Manhuaes_exports, {
    Manhuaes: () => Manhuaes,
    ManhuaesInfo: () => ManhuaesInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Manhuaes/ManhuaesParser.ts
  ...
  // src/Manhuaes/ManhuaesSettings.ts
  ...
  // src/Manhuaes/Manhuaes.ts
  var MANHUAES_DOMAIN = "https://manhuaes.com";
  var ManhuaesInfo = {
    version: "1.0.0",
    name: "Manhua ES",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Manhua ES",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MANHUAES_DOMAIN,
  ...
  var Manhuaes = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MANHUAES_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANHUAES_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANHUAES_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANHUAES_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Manhuaes_exports);
})();
