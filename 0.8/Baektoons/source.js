...
var _Sources = (() => {
  ...
  // src/Baektoons/Baektoons.ts
  var Baektoons_exports = {};
  __export(Baektoons_exports, {
    Baektoons: () => Baektoons,
    BaektoonsInfo: () => BaektoonsInfo
  });
  var import_types = __toESM(require_lib());

  // src/Baektoons/BaektoonsParser.ts
  ...
  // src/Baektoons/BaektoonsSettings.ts
  ...
  // src/Baektoons/Baektoons.ts
  var BAEKTOONS_DOMAIN = "https://baektoons.com";
  var BaektoonsInfo = {
    version: "1.0.0",
    name: "Baek Toons",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Baek Toons",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: BAEKTOONS_DOMAIN,
  ...
  var Baektoons = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${BAEKTOONS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${BAEKTOONS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${BAEKTOONS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${BAEKTOONS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${BAEKTOONS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Baektoons_exports);
})();
