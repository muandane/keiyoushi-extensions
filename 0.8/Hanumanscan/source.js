...
var _Sources = (() => {
  ...
  // src/Hanumanscan/Hanumanscan.ts
  var Hanumanscan_exports = {};
  __export(Hanumanscan_exports, {
    Hanumanscan: () => Hanumanscan,
    HanumanscanInfo: () => HanumanscanInfo
  });
  var import_types = __toESM(require_lib());

  // src/Hanumanscan/HanumanscanParser.ts
  ...
  // src/Hanumanscan/HanumanscanSettings.ts
  ...
  // src/Hanumanscan/Hanumanscan.ts
  var HANUMANSCAN_DOMAIN = "https://hanumanscan.com";
  var HanumanscanInfo = {
    version: "1.0.0",
    name: "Hanuman Scan",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Hanuman Scan",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: HANUMANSCAN_DOMAIN,
  ...
  var Hanumanscan = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${HANUMANSCAN_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${HANUMANSCAN_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${HANUMANSCAN_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${HANUMANSCAN_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${HANUMANSCAN_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Hanumanscan_exports);
})();
