...
var _Sources = (() => {
  ...
  // src/Aurora/Aurora.ts
  var AURORA_DOMAIN = "https://comicaurora.com";
  var AuroraInfo = {
    version: "1.0.0",
    name: "Aurora",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Aurora",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: AURORA_DOMAIN,
  ...
  var Aurora = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${AURORA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${AURORA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${AURORA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${AURORA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Aurora_exports);
})();
