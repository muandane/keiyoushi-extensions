...
var _Sources = (() => {
  ...
  // src/Wuxiaworld/Wuxiaworld.ts
  var Wuxiaworld_exports = {};
  __export(Wuxiaworld_exports, {
    Wuxiaworld: () => Wuxiaworld,
    WuxiaworldInfo: () => WuxiaworldInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Wuxiaworld/WuxiaworldParser.ts
  ...
  // src/Wuxiaworld/WuxiaworldSettings.ts
  ...
  // src/Wuxiaworld/Wuxiaworld.ts
  var WUXIAWORLD_DOMAIN = "https://wuxiaworld.site";
  var WuxiaworldInfo = {
    version: "1.0.0",
    name: "WuxiaWorld",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from WuxiaWorld",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: WUXIAWORLD_DOMAIN,
  ...
  var Wuxiaworld = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${WUXIAWORLD_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${WUXIAWORLD_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${WUXIAWORLD_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${WUXIAWORLD_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${WUXIAWORLD_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Wuxiaworld_exports);
})();
