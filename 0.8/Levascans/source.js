...
var _Sources = (() => {
  ...
  // src/Levascans/Levascans.ts
  var Levascans_exports = {};
  __export(Levascans_exports, {
    Levascans: () => Levascans,
    LevascansInfo: () => LevascansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Levascans/LevascansParser.ts
  ...
  // src/Levascans/LevascansSettings.ts
  ...
  // src/Levascans/Levascans.ts
  var LEVASCANS_DOMAIN = "https://levascans.com";
  var LevascansInfo = {
    version: "1.0.0",
    name: "Leva Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Leva Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: LEVASCANS_DOMAIN,
  ...
  var Levascans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${LEVASCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${LEVASCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${LEVASCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${LEVASCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${LEVASCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Levascans_exports);
})();
