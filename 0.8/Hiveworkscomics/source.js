...
var _Sources = (() => {
  ...
  // src/Hiveworkscomics/Hiveworkscomics.ts
  var Hiveworkscomics_exports = {};
  __export(Hiveworkscomics_exports, {
    Hiveworkscomics: () => Hiveworkscomics,
    HiveworkscomicsInfo: () => HiveworkscomicsInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Hiveworkscomics/HiveworkscomicsParser.ts
  ...
  // src/Hiveworkscomics/HiveworkscomicsSettings.ts
  ...
  // src/Hiveworkscomics/Hiveworkscomics.ts
  var HIVEWORKSCOMICS_DOMAIN = "https://hiveworkscomics.com";
  var HiveworkscomicsInfo = {
    version: "1.0.0",
    name: "Hiveworks Comics",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Hiveworks Comics",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: HIVEWORKSCOMICS_DOMAIN,
  ...
  var Hiveworkscomics = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${HIVEWORKSCOMICS_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${HIVEWORKSCOMICS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${HIVEWORKSCOMICS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${HIVEWORKSCOMICS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${HIVEWORKSCOMICS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${HIVEWORKSCOMICS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Hiveworkscomics_exports);
})();
