...
var _Sources = (() => {
  ...
  // src/Mangamo/Mangamo.ts
  var Mangamo_exports = {};
  __export(Mangamo_exports, {
    Mangamo: () => Mangamo,
    MangamoInfo: () => MangamoInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Mangamo/MangamoParser.ts
  ...
  // src/Mangamo/MangamoSettings.ts
  ...
  // src/Mangamo/Mangamo.ts
  var MANGAMO_DOMAIN = "https://www.mangamo.com";
  var MangamoInfo = {
    version: "1.0.0",
    name: "Mangamo",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Mangamo",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MANGAMO_DOMAIN,
  ...
  var Mangamo = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MANGAMO_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANGAMO_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGAMO_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGAMO_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Mangamo_exports);
})();
