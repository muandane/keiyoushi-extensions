...
var _Sources = (() => {
  ...
  // src/Holonometria/Holonometria.ts
  var Holonometria_exports = {};
  __export(Holonometria_exports, {
    Holonometria: () => Holonometria,
    HolonometriaInfo: () => HolonometriaInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Holonometria/HolonometriaParser.ts
  ...
  // src/Holonometria/HolonometriaSettings.ts
  ...
  // src/Holonometria/Holonometria.ts
  var HOLONOMETRIA_DOMAIN = "https://holoearth.com";
  var HolonometriaInfo = {
    version: "1.0.0",
    name: "HOLONOMETRIA",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from HOLONOMETRIA",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: HOLONOMETRIA_DOMAIN,
  ...
  var Holonometria = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${HOLONOMETRIA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${HOLONOMETRIA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${HOLONOMETRIA_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${HOLONOMETRIA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${HOLONOMETRIA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Holonometria_exports);
})();
