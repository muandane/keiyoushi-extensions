...
var _Sources = (() => {
  ...
  // src/Mangasushi/Mangasushi.ts
  var Mangasushi_exports = {};
  __export(Mangasushi_exports, {
    Mangasushi: () => Mangasushi,
    MangasushiInfo: () => MangasushiInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Mangasushi/MangasushiParser.ts
  ...
  // src/Mangasushi/MangasushiSettings.ts
  ...
  // src/Mangasushi/Mangasushi.ts
  var MANGASUSHI_DOMAIN = "https://mangasushi.org";
  var MangasushiInfo = {
    version: "1.0.0",
    name: "Mangasushi",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Mangasushi",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MANGASUSHI_DOMAIN,
  ...
  var Mangasushi = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MANGASUSHI_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANGASUSHI_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MANGASUSHI_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGASUSHI_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGASUSHI_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Mangasushi_exports);
})();
