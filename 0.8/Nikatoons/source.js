...
var _Sources = (() => {
  ...
  // src/Nikatoons/Nikatoons.ts
  var Nikatoons_exports = {};
  __export(Nikatoons_exports, {
    Nikatoons: () => Nikatoons,
    NikatoonsInfo: () => NikatoonsInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Nikatoons/NikatoonsParser.ts
  ...
  // src/Nikatoons/NikatoonsSettings.ts
  ...
  // src/Nikatoons/Nikatoons.ts
  var NIKATOONS_DOMAIN = "https://nikatoons.com";
  var NikatoonsInfo = {
    version: "1.0.0",
    name: "Nika Toons",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Nika Toons",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: NIKATOONS_DOMAIN,
  ...
  var Nikatoons = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${NIKATOONS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${NIKATOONS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${NIKATOONS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${NIKATOONS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${NIKATOONS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Nikatoons_exports);
})();
