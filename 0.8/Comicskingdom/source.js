...
var _Sources = (() => {
  ...
  // src/Comicskingdom/Comicskingdom.ts
  var Comicskingdom_exports = {};
  __export(Comicskingdom_exports, {
    Comicskingdom: () => Comicskingdom,
    ComicskingdomInfo: () => ComicskingdomInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Comicskingdom/ComicskingdomParser.ts
  ...
  // src/Comicskingdom/ComicskingdomSettings.ts
  ...
  // src/Comicskingdom/Comicskingdom.ts
  var COMICSKINGDOM_DOMAIN = "https://wp.comicskingdom.com";
  var ComicskingdomInfo = {
    version: "1.0.0",
    name: "Comics Kingdom",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Comics Kingdom",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: COMICSKINGDOM_DOMAIN,
  ...
  var Comicskingdom = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${COMICSKINGDOM_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${COMICSKINGDOM_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${COMICSKINGDOM_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${COMICSKINGDOM_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${COMICSKINGDOM_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Comicskingdom_exports);
})();
