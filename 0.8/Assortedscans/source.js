...
var _Sources = (() => {
  ...
  // src/Assortedscans/Assortedscans.ts
  var Assortedscans_exports = {};
  __export(Assortedscans_exports, {
    Assortedscans: () => Assortedscans,
    AssortedscansInfo: () => AssortedscansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Assortedscans/AssortedscansParser.ts
  ...
  // src/Assortedscans/AssortedscansSettings.ts
  ...
  // src/Assortedscans/Assortedscans.ts
  var ASSORTEDSCANS_DOMAIN = "https://assortedscans.com";
  var AssortedscansInfo = {
    version: "1.0.0",
    name: "Assorted Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Assorted Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: ASSORTEDSCANS_DOMAIN,
  ...
  var Assortedscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${ASSORTEDSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${ASSORTEDSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${ASSORTEDSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${ASSORTEDSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${ASSORTEDSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Assortedscans_exports);
})();
