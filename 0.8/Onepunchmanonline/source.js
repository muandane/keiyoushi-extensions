...
var _Sources = (() => {
  ...
  // src/Onepunchmanonline/Onepunchmanonline.ts
  var Onepunchmanonline_exports = {};
  __export(Onepunchmanonline_exports, {
    Onepunchmanonline: () => Onepunchmanonline,
    OnepunchmanonlineInfo: () => OnepunchmanonlineInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Onepunchmanonline/OnepunchmanonlineParser.ts
  ...
  // src/Onepunchmanonline/OnepunchmanonlineSettings.ts
  ...
  // src/Onepunchmanonline/Onepunchmanonline.ts
  var ONEPUNCHMANONLINE_DOMAIN = "https://w10.1punchman.com";
  var OnepunchmanonlineInfo = {
    version: "1.0.0",
    name: "One Punch Man Online",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from One Punch Man Online",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: ONEPUNCHMANONLINE_DOMAIN,
  ...
  var Onepunchmanonline = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${ONEPUNCHMANONLINE_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${ONEPUNCHMANONLINE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${ONEPUNCHMANONLINE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${ONEPUNCHMANONLINE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${ONEPUNCHMANONLINE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${ONEPUNCHMANONLINE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Onepunchmanonline_exports);
})();
