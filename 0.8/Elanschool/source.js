...
var _Sources = (() => {
  ...
  // src/Elanschool/Elanschool.ts
  var Elanschool_exports = {};
  __export(Elanschool_exports, {
    Elanschool: () => Elanschool,
    ElanschoolInfo: () => ElanschoolInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Elanschool/ElanschoolParser.ts
  ...
  // src/Elanschool/ElanschoolSettings.ts
  ...
  // src/Elanschool/Elanschool.ts
  var ELANSCHOOL_DOMAIN = "https://elan.school";
  var ElanschoolInfo = {
    version: "1.0.0",
    name: "Elan School",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Elan School",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: ELANSCHOOL_DOMAIN,
  ...
  var Elanschool = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${ELANSCHOOL_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${ELANSCHOOL_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${ELANSCHOOL_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${ELANSCHOOL_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${ELANSCHOOL_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Elanschool_exports);
})();
