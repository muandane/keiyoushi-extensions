...
var _Sources = (() => {
  ...
  // src/Ryumanga/Ryumanga.ts
  var Ryumanga_exports = {};
  __export(Ryumanga_exports, {
    Ryumanga: () => Ryumanga,
    RyumangaInfo: () => RyumangaInfo
  });
  var import_types = __toESM(require_lib());

  // src/Ryumanga/RyumangaParser.ts
  ...
  // src/Ryumanga/RyumangaSettings.ts
  ...
  // src/Ryumanga/Ryumanga.ts
  var RYUMANGA_DOMAIN = "https://ryumanga.org";
  var RyumangaInfo = {
    version: "1.0.0",
    name: "Ryumanga",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Ryumanga",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: RYUMANGA_DOMAIN,
  ...
  var Ryumanga = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${RYUMANGA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${RYUMANGA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${RYUMANGA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${RYUMANGA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Ryumanga_exports);
})();
