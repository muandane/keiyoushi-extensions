...
var _Sources = (() => {
  ...
  // src/Thunderscans/ThunderscansParser.ts
  ...
  // src/Thunderscans/ThunderscansSettings.ts
  ...
  // src/Thunderscans/Thunderscans.ts
  var THUNDERSCANS_DOMAIN = "https://en-thunderscans.com";
  var ThunderscansInfo = {
    ...
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Thunder Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: THUNDERSCANS_DOMAIN,
  ...
  var Thunderscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${THUNDERSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${THUNDERSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${THUNDERSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${THUNDERSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${THUNDERSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
