...
var _Sources = (() => {
  ...
  // src/Mayotune/Mayotune.ts
  var Mayotune_exports = {};
  __export(Mayotune_exports, {
    Mayotune: () => Mayotune,
    MayotuneInfo: () => MayotuneInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Mayotune/MayotuneParser.ts
  ...
  // src/Mayotune/MayotuneSettings.ts
  ...
  // src/Mayotune/Mayotune.ts
  var MAYOTUNE_DOMAIN = "https://mayotune.xyz";
  var MayotuneInfo = {
    version: "1.0.0",
    name: "MayoTune",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from MayoTune",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MAYOTUNE_DOMAIN,
  ...
  var Mayotune = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MAYOTUNE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MAYOTUNE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MAYOTUNE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MAYOTUNE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Mayotune_exports);
})();
