...
var _Sources = (() => {
  ...
  // src/Vizmanga/Vizmanga.ts
  var Vizmanga_exports = {};
  __export(Vizmanga_exports, {
    Vizmanga: () => Vizmanga,
    VizmangaInfo: () => VizmangaInfo
  });
  var import_types = __toESM(require_lib());

  // src/Vizmanga/VizmangaParser.ts
  ...
  // src/Vizmanga/VizmangaSettings.ts
  ...
  // src/Vizmanga/Vizmanga.ts
  var VIZMANGA_DOMAIN = "https://www.viz.com";
  var VizmangaInfo = {
    version: "1.0.0",
    name: "VIZ Manga",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from VIZ Manga",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: VIZMANGA_DOMAIN,
  ...
  var Vizmanga = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${VIZMANGA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${VIZMANGA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${VIZMANGA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${VIZMANGA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Vizmanga_exports);
})();
