...
var _Sources = (() => {
  ...
  // src/Manhuahot/Manhuahot.ts
  var Manhuahot_exports = {};
  __export(Manhuahot_exports, {
    Manhuahot: () => Manhuahot,
    ManhuahotInfo: () => ManhuahotInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Manhuahot/ManhuahotParser.ts
  ...
  // src/Manhuahot/ManhuahotSettings.ts
  ...
  // src/Manhuahot/Manhuahot.ts
  var MANHUAHOT_DOMAIN = "https://manhuahot.com";
  var ManhuahotInfo = {
    version: "1.0.0",
    name: "ManhuaHot",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from ManhuaHot",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MANHUAHOT_DOMAIN,
  ...
  var Manhuahot = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MANHUAHOT_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANHUAHOT_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MANHUAHOT_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANHUAHOT_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANHUAHOT_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Manhuahot_exports);
})();
