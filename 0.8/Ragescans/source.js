...
var _Sources = (() => {
  ...
  // src/Ragescans/Ragescans.ts
  var Ragescans_exports = {};
  __export(Ragescans_exports, {
    Ragescans: () => Ragescans,
    RagescansInfo: () => RagescansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Ragescans/RagescansParser.ts
  ...
  // src/Ragescans/RagescansSettings.ts
  ...
  // src/Ragescans/Ragescans.ts
  var RAGESCANS_DOMAIN = "https://ragescans.com";
  var RagescansInfo = {
    version: "1.0.0",
    name: "Rage Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Rage Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: RAGESCANS_DOMAIN,
  ...
  var Ragescans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${RAGESCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${RAGESCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${RAGESCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${RAGESCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${RAGESCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Ragescans_exports);
})();
