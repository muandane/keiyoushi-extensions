...
var _Sources = (() => {
  ...
  // src/Zeroscans/Zeroscans.ts
  var Zeroscans_exports = {};
  __export(Zeroscans_exports, {
    Zeroscans: () => Zeroscans,
    ZeroscansInfo: () => ZeroscansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Zeroscans/ZeroscansParser.ts
  ...
  // src/Zeroscans/ZeroscansSettings.ts
  ...
  // src/Zeroscans/Zeroscans.ts
  var ZEROSCANS_DOMAIN = "https://zscans.com";
  var ZeroscansInfo = {
    version: "1.0.0",
    name: "Zero Scans",
    icon: "icon.png",
  ...
  var Zeroscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${ZEROSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${ZEROSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${ZEROSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${ZEROSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${ZEROSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Zeroscans_exports);
})();
