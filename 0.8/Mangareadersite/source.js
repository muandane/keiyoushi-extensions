...
var _Sources = (() => {
  ...
  // src/Mangareadersite/Mangareadersite.ts
  var Mangareadersite_exports = {};
  __export(Mangareadersite_exports, {
    Mangareadersite: () => Mangareadersite,
    MangareadersiteInfo: () => MangareadersiteInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Mangareadersite/MangareadersiteParser.ts
  ...
  // src/Mangareadersite/MangareadersiteSettings.ts
  ...
  // src/Mangareadersite/Mangareadersite.ts
  var MANGAREADERSITE_DOMAIN = "https://mangareader.site";
  var MangareadersiteInfo = {
    version: "1.0.0",
    name: "MangaReader.site",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from MangaReader.site",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MANGAREADERSITE_DOMAIN,
  ...
  var Mangareadersite = class {
    ...
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANGAREADERSITE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MANGAREADERSITE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGAREADERSITE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGAREADERSITE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Mangareadersite_exports);
})();
