...
var _Sources = (() => {
  ...
  // src/Grrlpowercomic/Grrlpowercomic.ts
  var Grrlpowercomic_exports = {};
  __export(Grrlpowercomic_exports, {
    Grrlpowercomic: () => Grrlpowercomic,
    GrrlpowercomicInfo: () => GrrlpowercomicInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Grrlpowercomic/GrrlpowercomicParser.ts
  ...
  var parseHomeSections = ($, sectionCallback) => {
    ...
    for (const element of mangaElements.slice(0, 20)) {
      const title = $('[class*="comic"] h2', element).first().text().trim();
  ...
  // src/Grrlpowercomic/GrrlpowercomicSettings.ts
  ...
  // src/Grrlpowercomic/Grrlpowercomic.ts
  var GRRLPOWERCOMIC_DOMAIN = "https://www.grrlpowercomic.com";
  var GrrlpowercomicInfo = {
    version: "1.0.0",
    name: "Grrl Power Comic",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Grrl Power Comic",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: GRRLPOWERCOMIC_DOMAIN,
  ...
  var Grrlpowercomic = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${GRRLPOWERCOMIC_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${GRRLPOWERCOMIC_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${GRRLPOWERCOMIC_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${GRRLPOWERCOMIC_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${GRRLPOWERCOMIC_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Grrlpowercomic_exports);
})();
