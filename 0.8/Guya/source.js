...
var _Sources = (() => {
  ...
  // src/Guya/Guya.ts
  var Guya_exports = {};
  __export(Guya_exports, {
    Guya: () => Guya,
    GuyaInfo: () => GuyaInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Guya/GuyaParser.ts
  ...
  // src/Guya/Guya.ts
  var GUYA_DOMAIN = "https://guya.cubari.moe";
  var GuyaInfo = {
    version: "1.0.0",
    name: "Guya",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Guya",
  ...
  var Guya = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${GUYA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${GUYA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${GUYA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${GUYA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Guya_exports);
})();
