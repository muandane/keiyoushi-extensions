...
var _Sources = (() => {
  ...
  // src/Bakkinselfhosted/Bakkinselfhosted.ts
  var Bakkinselfhosted_exports = {};
  __export(Bakkinselfhosted_exports, {
    Bakkinselfhosted: () => Bakkinselfhosted,
    BakkinselfhostedInfo: () => BakkinselfhostedInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Bakkinselfhosted/BakkinselfhostedParser.ts
  ...
  // src/Bakkinselfhosted/BakkinselfhostedSettings.ts
  ...
  // src/Bakkinselfhosted/Bakkinselfhosted.ts
  var BAKKINSELFHOSTED_DOMAIN = "http://127.0.0.1";
  var BakkinselfhostedInfo = {
    version: "1.0.0",
    name: "Bakkin Self-hosted",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Bakkin Self-hosted",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: BAKKINSELFHOSTED_DOMAIN,
  ...
  var Bakkinselfhosted = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${BAKKINSELFHOSTED_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${BAKKINSELFHOSTED_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${BAKKINSELFHOSTED_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${BAKKINSELFHOSTED_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${BAKKINSELFHOSTED_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${BAKKINSELFHOSTED_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Bakkinselfhosted_exports);
})();
