...
var _Sources = (() => {
  ...
  // src/Clowncorps/Clowncorps.ts
  var Clowncorps_exports = {};
  __export(Clowncorps_exports, {
    Clowncorps: () => Clowncorps,
    ClowncorpsInfo: () => ClowncorpsInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Clowncorps/ClowncorpsParser.ts
  ...
  var parseHomeSections = ($, sectionCallback) => {
    ...
    for (const element of mangaElements.slice(0, 20)) {
      const title = $('[class*="comic"] h1', element).first().text().trim();
  ...
  // src/Clowncorps/ClowncorpsSettings.ts
  ...
  // src/Clowncorps/Clowncorps.ts
  var CLOWNCORPS_DOMAIN = "https://clowncorps.net";
  var ClowncorpsInfo = {
    version: "1.0.0",
    name: "Clown Corps",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Clown Corps",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: CLOWNCORPS_DOMAIN,
  ...
  var Clowncorps = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${CLOWNCORPS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${CLOWNCORPS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${CLOWNCORPS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${CLOWNCORPS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${CLOWNCORPS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Clowncorps_exports);
})();
