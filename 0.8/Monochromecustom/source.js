...
var _Sources = (() => {
  ...
  // src/Monochromecustom/Monochromecustom.ts
  var Monochromecustom_exports = {};
  __export(Monochromecustom_exports, {
    Monochromecustom: () => Monochromecustom,
    MonochromecustomInfo: () => MonochromecustomInfo
  });
  var import_types = __toESM(require_lib());

  // src/Monochromecustom/MonochromecustomParser.ts
  ...
  // src/Monochromecustom/MonochromecustomSettings.ts
  ...
  // src/Monochromecustom/Monochromecustom.ts
  var MONOCHROMECUSTOM_DOMAIN = "https://monochromecms.netlify.app";
  var MonochromecustomInfo = {
    version: "1.0.0",
    name: "Monochrome Custom",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Monochrome Custom",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: MONOCHROMECUSTOM_DOMAIN,
  ...
  var Monochromecustom = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${MONOCHROMECUSTOM_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${MONOCHROMECUSTOM_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MONOCHROMECUSTOM_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MONOCHROMECUSTOM_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MONOCHROMECUSTOM_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MONOCHROMECUSTOM_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Monochromecustom_exports);
})();
