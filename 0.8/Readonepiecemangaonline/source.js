...
var _Sources = (() => {
  ...
  // src/Readonepiecemangaonline/Readonepiecemangaonline.ts
  var Readonepiecemangaonline_exports = {};
  __export(Readonepiecemangaonline_exports, {
    Readonepiecemangaonline: () => Readonepiecemangaonline,
    ReadonepiecemangaonlineInfo: () => ReadonepiecemangaonlineInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Readonepiecemangaonline/ReadonepiecemangaonlineParser.ts
  ...
  // src/Readonepiecemangaonline/ReadonepiecemangaonlineSettings.ts
  ...
  // src/Readonepiecemangaonline/Readonepiecemangaonline.ts
  var READONEPIECEMANGAONLINE_DOMAIN = "https://ww12.readonepiece.com";
  var ReadonepiecemangaonlineInfo = {
    version: "1.0.0",
    name: "Read One Piece Manga Online",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Read One Piece Manga Online",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: READONEPIECEMANGAONLINE_DOMAIN,
  ...
  var Readonepiecemangaonline = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${READONEPIECEMANGAONLINE_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${READONEPIECEMANGAONLINE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${READONEPIECEMANGAONLINE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${READONEPIECEMANGAONLINE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: READONEPIECEMANGAONLINE_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${READONEPIECEMANGAONLINE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${READONEPIECEMANGAONLINE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Readonepiecemangaonline_exports);
})();
