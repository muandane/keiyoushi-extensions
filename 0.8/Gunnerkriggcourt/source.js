...
var _Sources = (() => {
  ...
  // src/Gunnerkriggcourt/Gunnerkriggcourt.ts
  var Gunnerkriggcourt_exports = {};
  __export(Gunnerkriggcourt_exports, {
    Gunnerkriggcourt: () => Gunnerkriggcourt,
    GunnerkriggcourtInfo: () => GunnerkriggcourtInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Gunnerkriggcourt/GunnerkriggcourtParser.ts
  ...
  var parseChapters = ($, mangaId) => {
    ...
    for (let i = 0; i < chapterElements.length; i++) {
      ...
      const title = $('[class*="chapter"] h3', element).text().trim() || `Chapter ${i + 1}`;
      const chapNum = i + 1;
  ...
  // src/Gunnerkriggcourt/GunnerkriggcourtSettings.ts
  ...
  // src/Gunnerkriggcourt/Gunnerkriggcourt.ts
  var GUNNERKRIGGCOURT_DOMAIN = "https://www.gunnerkrigg.com";
  var GunnerkriggcourtInfo = {
    version: "1.0.0",
    name: "Gunnerkrigg Court",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Gunnerkrigg Court",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: GUNNERKRIGGCOURT_DOMAIN,
  ...
  var Gunnerkriggcourt = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${GUNNERKRIGGCOURT_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${GUNNERKRIGGCOURT_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${GUNNERKRIGGCOURT_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${GUNNERKRIGGCOURT_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${GUNNERKRIGGCOURT_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${GUNNERKRIGGCOURT_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Gunnerkriggcourt_exports);
})();
