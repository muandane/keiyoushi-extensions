...
var _Sources = (() => {
  ...
  // src/Monochromescans/Monochromescans.ts
  var Monochromescans_exports = {};
  __export(Monochromescans_exports, {
    Monochromescans: () => Monochromescans,
    MonochromescansInfo: () => MonochromescansInfo
  });
  var import_types = __toESM(require_lib());

  // src/Monochromescans/MonochromescansParser.ts
  ...
  // src/Monochromescans/MonochromescansSettings.ts
  ...
  // src/Monochromescans/Monochromescans.ts
  var MONOCHROMESCANS_DOMAIN = "https://manga.d34d.one";
  var MonochromescansInfo = {
    version: "1.0.0",
    name: "Monochrome Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Monochrome Scans",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: MONOCHROMESCANS_DOMAIN,
  ...
  var Monochromescans = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${MONOCHROMESCANS_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${MONOCHROMESCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MONOCHROMESCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MONOCHROMESCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MONOCHROMESCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MONOCHROMESCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Monochromescans_exports);
})();
