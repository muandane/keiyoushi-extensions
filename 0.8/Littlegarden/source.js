...
var _Sources = (() => {
  ...
  // src/Littlegarden/Littlegarden.ts
  var Littlegarden_exports = {};
  __export(Littlegarden_exports, {
    Littlegarden: () => Littlegarden,
    LittlegardenInfo: () => LittlegardenInfo
  });
  var import_types = __toESM(require_lib());

  // src/Littlegarden/LittlegardenParser.ts
  ...
  // src/Littlegarden/LittlegardenSettings.ts
  ...
  // src/Littlegarden/Littlegarden.ts
  var LITTLEGARDEN_DOMAIN = "https://littlexgarden.com";
  var LittlegardenInfo = {
    version: "1.0.0",
    name: "Little Garden",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Little Garden",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: LITTLEGARDEN_DOMAIN,
  ...
  var Littlegarden = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${LITTLEGARDEN_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${LITTLEGARDEN_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${LITTLEGARDEN_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${LITTLEGARDEN_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${LITTLEGARDEN_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Littlegarden_exports);
})();
