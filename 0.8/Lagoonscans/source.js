...
var _Sources = (() => {
  ...
  // src/Lagoonscans/Lagoonscans.ts
  var Lagoonscans_exports = {};
  __export(Lagoonscans_exports, {
    Lagoonscans: () => Lagoonscans,
    LagoonscansInfo: () => LagoonscansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Lagoonscans/LagoonscansParser.ts
  ...
  var parseHomeSections = ($, sectionCallback) => {
    ...
    for (const element of mangaElements.slice(0, 20)) {
      const title = $('[class*="series"] .title, [class*="series"] .name, [class*="series"] h3', element).first().text().trim();
      const image = $('[class*="series"] img', element).first().attr("src") || $('[class*="series"] img', element).first().attr("data-src") || "";
      const link = $('[class*="series"]', element).first().attr("href") || "";
  ...
  // src/Lagoonscans/LagoonscansSettings.ts
  ...
  // src/Lagoonscans/Lagoonscans.ts
  var LAGOONSCANS_DOMAIN = "https://lagoonscans.com";
  var LagoonscansInfo = {
    version: "1.0.0",
    name: "Lagoon Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Lagoon Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: LAGOONSCANS_DOMAIN,
  ...
  var Lagoonscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${LAGOONSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${LAGOONSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${LAGOONSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${LAGOONSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${LAGOONSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Lagoonscans_exports);
})();
