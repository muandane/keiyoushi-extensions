...
var _Sources = (() => {
  ...
  // src/Boratscans/Boratscans.ts
  var Boratscans_exports = {};
  __export(Boratscans_exports, {
    Boratscans: () => Boratscans,
    BoratscansInfo: () => BoratscansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Boratscans/BoratscansParser.ts
  ...
  // src/Boratscans/BoratscansSettings.ts
  ...
  // src/Boratscans/Boratscans.ts
  var BORATSCANS_DOMAIN = "https://boratscans.com";
  var BoratscansInfo = {
    version: "1.0.0",
    name: "Borat Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Borat Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: BORATSCANS_DOMAIN,
  ...
  var Boratscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${BORATSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${BORATSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${BORATSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${BORATSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${BORATSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Boratscans_exports);
})();
