...
var _Sources = (() => {
  ...
  // src/Writerscans/Writerscans.ts
  var Writerscans_exports = {};
  __export(Writerscans_exports, {
    Writerscans: () => Writerscans,
    WriterscansInfo: () => WriterscansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Writerscans/WriterscansParser.ts
  ...
  // src/Writerscans/WriterscansSettings.ts
  ...
  // src/Writerscans/Writerscans.ts
  var WRITERSCANS_DOMAIN = "https://writerscans.com";
  var WriterscansInfo = {
    version: "1.0.0",
    name: "Writer Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Writer Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: WRITERSCANS_DOMAIN,
  ...
  var Writerscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${WRITERSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${WRITERSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${WRITERSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${WRITERSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${WRITERSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Writerscans_exports);
})();
