...
var _Sources = (() => {
  ...
  // src/Rdscans/RdscansSettings.ts
  ...
  // src/Rdscans/Rdscans.ts
  var RDSCANS_DOMAIN = "https://rdscans.com";
  var RdscansInfo = {
    version: "1.0.0",
    name: "RD Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from RD Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: RDSCANS_DOMAIN,
  ...
  var Rdscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${RDSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${RDSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${RDSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${RDSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
