...
var _Sources = (() => {
  ...
  // src/Readcomicsonline/ReadcomicsonlineSettings.ts
  ...
  // src/Readcomicsonline/Readcomicsonline.ts
  var READCOMICSONLINE_DOMAIN = "https://readcomicsonline.ru";
  var ReadcomicsonlineInfo = {
    ...
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Read Comics Online",
  ...
  var Readcomicsonline = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${READCOMICSONLINE_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${READCOMICSONLINE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${READCOMICSONLINE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${READCOMICSONLINE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${READCOMICSONLINE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${READCOMICSONLINE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
