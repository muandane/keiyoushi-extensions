...
var _Sources = (() => {
  ...
  // src/Ezmanga/Ezmanga.ts
  var Ezmanga_exports = {};
  __export(Ezmanga_exports, {
    Ezmanga: () => Ezmanga,
    EzmangaInfo: () => EzmangaInfo
  });
  var import_types = __toESM(require_lib());

  // src/Ezmanga/EzmangaParser.ts
  ...
  // src/Ezmanga/EzmangaSettings.ts
  ...
  // src/Ezmanga/Ezmanga.ts
  var EZMANGA_DOMAIN = "https://ezmanga.org";
  var EzmangaInfo = {
    version: "1.0.0",
    name: "EZmanga",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from EZmanga",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: EZMANGA_DOMAIN,
  ...
  var Ezmanga = class {
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${EZMANGA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${EZMANGA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Ezmanga_exports);
})();
