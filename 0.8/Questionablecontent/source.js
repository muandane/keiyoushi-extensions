...
var _Sources = (() => {
  ...
  // src/Questionablecontent/Questionablecontent.ts
  var Questionablecontent_exports = {};
  __export(Questionablecontent_exports, {
    Questionablecontent: () => Questionablecontent,
    QuestionablecontentInfo: () => QuestionablecontentInfo
  });
  var import_types = __toESM(require_lib());

  // src/Questionablecontent/QuestionablecontentParser.ts
  ...
  // src/Questionablecontent/QuestionablecontentSettings.ts
  ...
  // src/Questionablecontent/Questionablecontent.ts
  var QUESTIONABLECONTENT_DOMAIN = "https://www.questionablecontent.net";
  var QuestionablecontentInfo = {
    version: "1.0.0",
    name: "Questionable Content",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Questionable Content",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: QUESTIONABLECONTENT_DOMAIN,
  ...
  var Questionablecontent = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${QUESTIONABLECONTENT_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${QUESTIONABLECONTENT_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${QUESTIONABLECONTENT_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${QUESTIONABLECONTENT_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: QUESTIONABLECONTENT_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${QUESTIONABLECONTENT_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${QUESTIONABLECONTENT_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Questionablecontent_exports);
})();
