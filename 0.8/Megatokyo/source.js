...
var _Sources = (() => {
  ...
  // src/Megatokyo/Megatokyo.ts
  var Megatokyo_exports = {};
  __export(Megatokyo_exports, {
    Megatokyo: () => Megatokyo,
    MegatokyoInfo: () => MegatokyoInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Megatokyo/MegatokyoParser.ts
  ...
  // src/Megatokyo/MegatokyoSettings.ts
  ...
  // src/Megatokyo/Megatokyo.ts
  var MEGATOKYO_DOMAIN = "https://megatokyo.com";
  var MegatokyoInfo = {
    version: "1.0.0",
    name: "Megatokyo",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Megatokyo",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MEGATOKYO_DOMAIN,
  ...
  var Megatokyo = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MEGATOKYO_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MEGATOKYO_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MEGATOKYO_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MEGATOKYO_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MEGATOKYO_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Megatokyo_exports);
})();
