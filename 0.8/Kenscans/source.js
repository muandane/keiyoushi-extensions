...
var _Sources = (() => {
  ...
  // src/Kenscans/Kenscans.ts
  var Kenscans_exports = {};
  __export(Kenscans_exports, {
    Kenscans: () => Kenscans,
    KenscansInfo: () => KenscansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Kenscans/KenscansParser.ts
  ...
  // src/Kenscans/KenscansSettings.ts
  ...
  // src/Kenscans/Kenscans.ts
  var KENSCANS_DOMAIN = "https://kencomics.com";
  var KenscansInfo = {
    version: "1.0.0",
    name: "Ken Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Ken Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: KENSCANS_DOMAIN,
  ...
  var Kenscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${KENSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${KENSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${KENSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${KENSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Kenscans_exports);
})();
