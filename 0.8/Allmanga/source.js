...
var _Sources = (() => {
  ...
  // src/Allmanga/Allmanga.ts
  var Allmanga_exports = {};
  __export(Allmanga_exports, {
    Allmanga: () => Allmanga,
    AllmangaInfo: () => AllmangaInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Allmanga/AllmangaParser.ts
  ...
  // src/Allmanga/AllmangaSettings.ts
  ...
  // src/Allmanga/Allmanga.ts
  var ALLMANGA_DOMAIN = "https://allmanga.to";
  var AllmangaInfo = {
    version: "1.0.0",
    name: "AllManga",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from AllManga",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: ALLMANGA_DOMAIN,
  ...
  var Allmanga = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${ALLMANGA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${ALLMANGA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${ALLMANGA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${ALLMANGA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Allmanga_exports);
})();
