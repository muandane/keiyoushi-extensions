...
var _Sources = (() => {
  ...
  // src/Manhuaplusunoriginal/Manhuaplusunoriginal.ts
  var Manhuaplusunoriginal_exports = {};
  __export(Manhuaplusunoriginal_exports, {
    Manhuaplusunoriginal: () => Manhuaplusunoriginal,
    ManhuaplusunoriginalInfo: () => ManhuaplusunoriginalInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Manhuaplusunoriginal/ManhuaplusunoriginalParser.ts
  ...
  var parseHomeSections = ($, sectionCallback) => {
    ...
    for (const element of mangaElements.slice(0, 20)) {
      const title = $('[class*="comic"] .name', element).first().text().trim();
  ...
  // src/Manhuaplusunoriginal/ManhuaplusunoriginalSettings.ts
  ...
  // src/Manhuaplusunoriginal/Manhuaplusunoriginal.ts
  var MANHUAPLUSUNORIGINAL_DOMAIN = "https://manhuaplus.org";
  var ManhuaplusunoriginalInfo = {
    version: "1.0.0",
    name: "ManhuaPlus (Unoriginal)",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from ManhuaPlus (Unoriginal)",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MANHUAPLUSUNORIGINAL_DOMAIN,
  ...
  var Manhuaplusunoriginal = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${MANHUAPLUSUNORIGINAL_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${MANHUAPLUSUNORIGINAL_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANHUAPLUSUNORIGINAL_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MANHUAPLUSUNORIGINAL_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: MANHUAPLUSUNORIGINAL_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANHUAPLUSUNORIGINAL_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANHUAPLUSUNORIGINAL_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Manhuaplusunoriginal_exports);
})();
