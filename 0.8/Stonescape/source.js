...
var _Sources = (() => {
  ...
  // src/Stonescape/StonescapeParser.ts
  ...
  // src/Stonescape/StonescapeSettings.ts
  ...
  // src/Stonescape/Stonescape.ts
  var STONESCAPE_DOMAIN = "https://stonescape.xyz";
  var StonescapeInfo = {
    version: "1.0.0",
    name: "StoneScape",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from StoneScape",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: STONESCAPE_DOMAIN,
  ...
  var Stonescape = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${STONESCAPE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${STONESCAPE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${STONESCAPE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${STONESCAPE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${STONESCAPE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
