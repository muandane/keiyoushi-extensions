...
var _Sources = (() => {
  ...
  var parseChapters = ($, mangaId) => {
    const chapters = [];
    const chapterElements = $(".chapters").toArray();
    for (let i = 0; i < chapterElements.length; i++) {
      const element = chapterElements[i];
      const chapterId = $(".chapters a", element).attr("href")?.replace(/\//g, "").split("/").filter(Boolean).pop() || `chapter-${i}`;
      const title = $(".chapters a", element).text().trim() || `Chapter ${i + 1}`;
      const chapNum = i + 1;
  ...
  var parseHomeSections = ($, sectionCallback) => {
    const latestItems = [];
    const mangaElements = $(".item").toArray();
    for (const element of mangaElements.slice(0, 20)) {
      const title = $(".item .title", element).first().text().trim();
      const image = $(".item img", element).first().attr("src") || $(".item img", element).first().attr("data-src") || "";
      const link = $(".item a", element).first().attr("href") || "";
  ...
  var parseViewMore = ($, homepageSectionId, metadata) => {
    ...
    for (const element of mangaElements) {
      const title = $(".item .title", element).first().text().trim();
      const image = $(".item img", element).first().attr("src") || "";
      const link = $(".item a", element).first().attr("href") || "";
      const mangaId = link.split("/").filter(Boolean).pop() || "";
      if (title && mangaId) {
  ...
  // src/Mangakatana/MangakatanaSettings.ts
  ...
  // src/Mangakatana/Mangakatana.ts
  var MANGAKATANA_DOMAIN = "https://mangakatana.com";
  var MangakatanaInfo = {
    version: "1.0.0",
    name: "MangaKatana",
    icon: "icon.png",
    ...
    websiteBaseURL: MANGAKATANA_DOMAIN,
  ...
  var Mangakatana = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MANGAKATANA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANGAKATANA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MANGAKATANA_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGAKATANA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGAKATANA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
