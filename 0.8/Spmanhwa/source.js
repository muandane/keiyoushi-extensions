...
var _Sources = (() => {
  ...
  // src/Spmanhwa/Spmanhwa.ts
  var Spmanhwa_exports = {};
  __export(Spmanhwa_exports, {
    Spmanhwa: () => Spmanhwa,
    SpmanhwaInfo: () => SpmanhwaInfo
  });
  var import_types = __toESM(require_lib());

  // src/Spmanhwa/SpmanhwaParser.ts
  ...
  // src/Spmanhwa/SpmanhwaSettings.ts
  ...
  // src/Spmanhwa/Spmanhwa.ts
  var SPMANHWA_DOMAIN = "https://spmanhwa.online";
  var SpmanhwaInfo = {
    version: "1.0.0",
    name: "Spmanhwa",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Spmanhwa",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: SPMANHWA_DOMAIN,
  ...
  var Spmanhwa = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${SPMANHWA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${SPMANHWA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${SPMANHWA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${SPMANHWA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Spmanhwa_exports);
})();
