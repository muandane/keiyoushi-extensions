...
var _Sources = (() => {
  ...
  // src/Readdrstonemangaonline/Readdrstonemangaonline.ts
  var Readdrstonemangaonline_exports = {};
  __export(Readdrstonemangaonline_exports, {
    Readdrstonemangaonline: () => Readdrstonemangaonline,
    ReaddrstonemangaonlineInfo: () => ReaddrstonemangaonlineInfo
  });
  var import_types = __toESM(require_lib());

  // src/Readdrstonemangaonline/ReaddrstonemangaonlineParser.ts
  ...
  // src/Readdrstonemangaonline/ReaddrstonemangaonlineSettings.ts
  ...
  // src/Readdrstonemangaonline/Readdrstonemangaonline.ts
  var READDRSTONEMANGAONLINE_DOMAIN = "https://ww7.readdrstone.com";
  var ReaddrstonemangaonlineInfo = {
    version: "1.0.0",
    name: "Read Dr. Stone Manga Online",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Read Dr. Stone Manga Online",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: READDRSTONEMANGAONLINE_DOMAIN,
  ...
  var Readdrstonemangaonline = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${READDRSTONEMANGAONLINE_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${READDRSTONEMANGAONLINE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${READDRSTONEMANGAONLINE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${READDRSTONEMANGAONLINE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: READDRSTONEMANGAONLINE_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${READDRSTONEMANGAONLINE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${READDRSTONEMANGAONLINE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Readdrstonemangaonline_exports);
})();
