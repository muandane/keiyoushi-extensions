...
var _Sources = (() => {
  ...
  // src/Wearehunger/Wearehunger.ts
  var Wearehunger_exports = {};
  __export(Wearehunger_exports, {
    Wearehunger: () => Wearehunger,
    WearehungerInfo: () => WearehungerInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Wearehunger/WearehungerParser.ts
  ...
  var parseHomeSections = ($, sectionCallback) => {
    ...
    for (const element of mangaElements.slice(0, 20)) {
      const title = $('[class*="manga"] h3', element).first().text().trim();
  ...
  // src/Wearehunger/WearehungerSettings.ts
  ...
  // src/Wearehunger/Wearehunger.ts
  var WEAREHUNGER_DOMAIN = "https://www.wearehunger.site";
  var WearehungerInfo = {
    version: "1.0.0",
    name: "Wearehunger",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Wearehunger",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: WEAREHUNGER_DOMAIN,
  ...
  var Wearehunger = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${WEAREHUNGER_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${WEAREHUNGER_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${WEAREHUNGER_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${WEAREHUNGER_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${WEAREHUNGER_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Wearehunger_exports);
})();
