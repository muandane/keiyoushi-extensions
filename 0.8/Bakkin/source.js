...
var _Sources = (() => {
  ...
  // src/Bakkin/Bakkin.ts
  var Bakkin_exports = {};
  __export(Bakkin_exports, {
    Bakkin: () => Bakkin,
    BakkinInfo: () => BakkinInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Bakkin/BakkinParser.ts
  ...
  var parseHomeSections = ($, sectionCallback) => {
    ...
    for (const element of mangaElements.slice(0, 20)) {
      const title = $("article .title", element).first().text().trim();
  ...
  // src/Bakkin/Bakkin.ts
  var BAKKIN_DOMAIN = "https://bakkin.moe";
  var BakkinInfo = {
    version: "1.0.0",
    name: "Bakkin",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Bakkin",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: BAKKIN_DOMAIN,
  ...
  var Bakkin = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${BAKKIN_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${BAKKIN_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${BAKKIN_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${BAKKIN_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Bakkin_exports);
})();
