...
var _Sources = (() => {
  ...
  // src/Noxenscans/Noxenscans.ts
  var Noxenscans_exports = {};
  __export(Noxenscans_exports, {
    Noxenscans: () => Noxenscans,
    NoxenscansInfo: () => NoxenscansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Noxenscans/NoxenscansParser.ts
  ...
  // src/Noxenscans/NoxenscansSettings.ts
  ...
  // src/Noxenscans/Noxenscans.ts
  var NOXENSCANS_DOMAIN = "https://noxenscan.com";
  var NoxenscansInfo = {
    version: "1.0.0",
    name: "Noxen Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Noxen Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: NOXENSCANS_DOMAIN,
  ...
  var Noxenscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${NOXENSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${NOXENSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${NOXENSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${NOXENSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${NOXENSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Noxenscans_exports);
})();
