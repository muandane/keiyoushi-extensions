...
var _Sources = (() => {
  ...
  // src/Cyanidehappiness/Cyanidehappiness.ts
  var Cyanidehappiness_exports = {};
  __export(Cyanidehappiness_exports, {
    Cyanidehappiness: () => Cyanidehappiness,
    CyanidehappinessInfo: () => CyanidehappinessInfo
  });
  var import_types = __toESM(require_lib());

  // src/Cyanidehappiness/CyanidehappinessParser.ts
  ...
  // src/Cyanidehappiness/CyanidehappinessSettings.ts
  ...
  // src/Cyanidehappiness/Cyanidehappiness.ts
  var CYANIDEHAPPINESS_DOMAIN = "https://explosm.net";
  var CyanidehappinessInfo = {
    version: "1.0.0",
    name: "Cyanide & Happiness",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Cyanide & Happiness",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: CYANIDEHAPPINESS_DOMAIN,
  ...
  var Cyanidehappiness = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${CYANIDEHAPPINESS_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${CYANIDEHAPPINESS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${CYANIDEHAPPINESS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${CYANIDEHAPPINESS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${CYANIDEHAPPINESS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${CYANIDEHAPPINESS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Cyanidehappiness_exports);
})();
