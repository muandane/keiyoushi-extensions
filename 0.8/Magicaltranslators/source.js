...
var _Sources = (() => {
  ...
  // src/Magicaltranslators/Magicaltranslators.ts
  var Magicaltranslators_exports = {};
  __export(Magicaltranslators_exports, {
    Magicaltranslators: () => Magicaltranslators,
    MagicaltranslatorsInfo: () => MagicaltranslatorsInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Magicaltranslators/MagicaltranslatorsParser.ts
  ...
  // src/Magicaltranslators/MagicaltranslatorsSettings.ts
  ...
  // src/Magicaltranslators/Magicaltranslators.ts
  var MAGICALTRANSLATORS_DOMAIN = "https://mahoushoujobu.com";
  var MagicaltranslatorsInfo = {
    version: "1.0.0",
    name: "Magical Translators",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Magical Translators",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MAGICALTRANSLATORS_DOMAIN,
  ...
  var Magicaltranslators = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${MAGICALTRANSLATORS_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${MAGICALTRANSLATORS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MAGICALTRANSLATORS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MAGICALTRANSLATORS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MAGICALTRANSLATORS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MAGICALTRANSLATORS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Magicaltranslators_exports);
})();
