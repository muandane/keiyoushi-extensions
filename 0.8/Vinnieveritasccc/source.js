...
var _Sources = (() => {
  ...
  // src/Vinnieveritasccc/Vinnieveritasccc.ts
  var Vinnieveritasccc_exports = {};
  __export(Vinnieveritasccc_exports, {
    Vinnieveritasccc: () => Vinnieveritasccc,
    VinnieveritascccInfo: () => VinnieveritascccInfo
  });
  var import_types = __toESM(require_lib());

  // src/Vinnieveritasccc/VinnieveritascccParser.ts
  ...
  // src/Vinnieveritasccc/VinnieveritascccSettings.ts
  ...
  // src/Vinnieveritasccc/Vinnieveritasccc.ts
  var VINNIEVERITASCCC_DOMAIN = "https://vinnieveritas.com";
  var VinnieveritascccInfo = {
    version: "1.0.0",
    name: "Vinnie Veritas - CCC",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Vinnie Veritas - CCC",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: VINNIEVERITASCCC_DOMAIN,
  ...
  var Vinnieveritasccc = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${VINNIEVERITASCCC_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${VINNIEVERITASCCC_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${VINNIEVERITASCCC_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${VINNIEVERITASCCC_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${VINNIEVERITASCCC_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${VINNIEVERITASCCC_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Vinnieveritasccc_exports);
})();
