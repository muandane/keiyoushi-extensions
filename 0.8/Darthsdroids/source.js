...
var _Sources = (() => {
  ...
  // src/Darthsdroids/Darthsdroids.ts
  var Darthsdroids_exports = {};
  __export(Darthsdroids_exports, {
    Darthsdroids: () => Darthsdroids,
    DarthsdroidsInfo: () => DarthsdroidsInfo
  });
  var import_types = __toESM(require_lib());

  // src/Darthsdroids/DarthsdroidsParser.ts
  ...
  // src/Darthsdroids/DarthsdroidsSettings.ts
  ...
  // src/Darthsdroids/Darthsdroids.ts
  var DARTHSDROIDS_DOMAIN = "https://www.darthsanddroids.net";
  var DarthsdroidsInfo = {
    version: "1.0.0",
    name: "Darths & Droids",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Darths & Droids",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: DARTHSDROIDS_DOMAIN,
  ...
  var Darthsdroids = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${DARTHSDROIDS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${DARTHSDROIDS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${DARTHSDROIDS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${DARTHSDROIDS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${DARTHSDROIDS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Darthsdroids_exports);
})();
