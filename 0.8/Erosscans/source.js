...
var _Sources = (() => {
  ...
  // src/Erosscans/Erosscans.ts
  var Erosscans_exports = {};
  __export(Erosscans_exports, {
    Erosscans: () => Erosscans,
    ErosscansInfo: () => ErosscansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Erosscans/ErosscansParser.ts
  ...
  // src/Erosscans/ErosscansSettings.ts
  ...
  // src/Erosscans/Erosscans.ts
  var EROSSCANS_DOMAIN = "https://erosvoid.xyz";
  var ErosscansInfo = {
    version: "1.0.0",
    name: "Eros Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Eros Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: EROSSCANS_DOMAIN,
  ...
  var Erosscans = class {
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${EROSSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${EROSSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${EROSSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Erosscans_exports);
})();
