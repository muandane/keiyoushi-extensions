...
var _Sources = (() => {
  ...
  // src/Flamescanslol/Flamescanslol.ts
  var Flamescanslol_exports = {};
  __export(Flamescanslol_exports, {
    Flamescanslol: () => Flamescanslol,
    FlamescanslolInfo: () => FlamescanslolInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Flamescanslol/FlamescanslolParser.ts
  ...
  // src/Flamescanslol/FlamescanslolSettings.ts
  ...
  // src/Flamescanslol/Flamescanslol.ts
  var FLAMESCANSLOL_DOMAIN = "https://flamescans.lol";
  var FlamescanslolInfo = {
    version: "1.0.0",
    name: "FlameScans.lol",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from FlameScans.lol",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: FLAMESCANSLOL_DOMAIN,
  ...
  var Flamescanslol = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${FLAMESCANSLOL_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${FLAMESCANSLOL_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${FLAMESCANSLOL_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${FLAMESCANSLOL_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${FLAMESCANSLOL_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Flamescanslol_exports);
})();
