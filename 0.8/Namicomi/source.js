...
var _Sources = (() => {
  ...
  // src/Namicomi/Namicomi.ts
  var Namicomi_exports = {};
  __export(Namicomi_exports, {
    Namicomi: () => Namicomi,
    NamicomiInfo: () => NamicomiInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Namicomi/NamicomiParser.ts
  ...
  // src/Namicomi/NamicomiSettings.ts
  ...
  // src/Namicomi/Namicomi.ts
  var NAMICOMI_DOMAIN = "https://namicomi.com";
  var NamicomiInfo = {
    version: "1.0.0",
    name: "NamiComi",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from NamiComi",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: NAMICOMI_DOMAIN,
  ...
  var Namicomi = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${NAMICOMI_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${NAMICOMI_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${NAMICOMI_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${NAMICOMI_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Namicomi_exports);
})();
