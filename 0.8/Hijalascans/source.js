...
var _Sources = (() => {
  ...
  // src/Hijalascans/Hijalascans.ts
  var Hijalascans_exports = {};
  __export(Hijalascans_exports, {
    Hijalascans: () => Hijalascans,
    HijalascansInfo: () => HijalascansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Hijalascans/HijalascansParser.ts
  ...
  // src/Hijalascans/HijalascansSettings.ts
  ...
  // src/Hijalascans/Hijalascans.ts
  var HIJALASCANS_DOMAIN = "https://en-hijala.com";
  var HijalascansInfo = {
    version: "1.0.0",
    name: "Hijala Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Hijala Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: HIJALASCANS_DOMAIN,
  ...
  var Hijalascans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${HIJALASCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${HIJALASCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${HIJALASCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${HIJALASCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${HIJALASCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Hijalascans_exports);
})();
