...
var _Sources = (() => {
  ...
  // src/Elarctoon/Elarctoon.ts
  var Elarctoon_exports = {};
  __export(Elarctoon_exports, {
    Elarctoon: () => Elarctoon,
    ElarctoonInfo: () => ElarctoonInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Elarctoon/ElarctoonParser.ts
  ...
  // src/Elarctoon/ElarctoonSettings.ts
  ...
  // src/Elarctoon/Elarctoon.ts
  var ELARCTOON_DOMAIN = "https://elarctoons.biz";
  var ElarctoonInfo = {
    version: "1.0.0",
    name: "Elarc Toon",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Elarc Toon",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: ELARCTOON_DOMAIN,
  ...
  var Elarctoon = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${ELARCTOON_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${ELARCTOON_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${ELARCTOON_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${ELARCTOON_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${ELARCTOON_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Elarctoon_exports);
})();
