...
var _Sources = (() => {
  ...
  // src/Manhuaus/ManhuausParser.ts
  ...
  // src/Manhuaus/ManhuausSettings.ts
  ...
  // src/Manhuaus/Manhuaus.ts
  var MANHUAUS_DOMAIN = "https://manhuaus.com";
  var ManhuausInfo = {
    version: "1.0.0",
    name: "ManhuaUS",
    icon: "icon.png",
  ...
  var Manhuaus = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MANHUAUS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANHUAUS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANHUAUS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANHUAUS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
