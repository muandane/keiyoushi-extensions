...
var _Sources = (() => {
  ...
  // src/Schlockmercenary/Schlockmercenary.ts
  var Schlockmercenary_exports = {};
  __export(Schlockmercenary_exports, {
    Schlockmercenary: () => Schlockmercenary,
    SchlockmercenaryInfo: () => SchlockmercenaryInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Schlockmercenary/SchlockmercenaryParser.ts
  ...
  // src/Schlockmercenary/SchlockmercenarySettings.ts
  ...
  // src/Schlockmercenary/Schlockmercenary.ts
  var SCHLOCKMERCENARY_DOMAIN = "https://www.schlockmercenary.com";
  var SchlockmercenaryInfo = {
    version: "1.0.0",
    name: "Schlock Mercenary",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Schlock Mercenary",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: SCHLOCKMERCENARY_DOMAIN,
  ...
  var Schlockmercenary = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${SCHLOCKMERCENARY_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${SCHLOCKMERCENARY_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${SCHLOCKMERCENARY_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${SCHLOCKMERCENARY_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${SCHLOCKMERCENARY_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${SCHLOCKMERCENARY_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Schlockmercenary_exports);
})();
