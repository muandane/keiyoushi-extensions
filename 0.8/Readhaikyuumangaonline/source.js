...
var _Sources = (() => {
  ...
  // src/Readhaikyuumangaonline/Readhaikyuumangaonline.ts
  var Readhaikyuumangaonline_exports = {};
  __export(Readhaikyuumangaonline_exports, {
    Readhaikyuumangaonline: () => Readhaikyuumangaonline,
    ReadhaikyuumangaonlineInfo: () => ReadhaikyuumangaonlineInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Readhaikyuumangaonline/ReadhaikyuumangaonlineParser.ts
  ...
  // src/Readhaikyuumangaonline/ReadhaikyuumangaonlineSettings.ts
  ...
  // src/Readhaikyuumangaonline/Readhaikyuumangaonline.ts
  var READHAIKYUUMANGAONLINE_DOMAIN = "https://ww9.readhaikyuu.com";
  var ReadhaikyuumangaonlineInfo = {
    version: "1.0.0",
    name: "Read Haikyuu!! Manga Online",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Read Haikyuu!! Manga Online",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: READHAIKYUUMANGAONLINE_DOMAIN,
  ...
  var Readhaikyuumangaonline = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${READHAIKYUUMANGAONLINE_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${READHAIKYUUMANGAONLINE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${READHAIKYUUMANGAONLINE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${READHAIKYUUMANGAONLINE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: READHAIKYUUMANGAONLINE_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${READHAIKYUUMANGAONLINE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${READHAIKYUUMANGAONLINE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Readhaikyuumangaonline_exports);
})();
