...
var _Sources = (() => {
  ...
  // src/Lhtranslation/LhtranslationParser.ts
  ...
  // src/Lhtranslation/LhtranslationSettings.ts
  ...
  // src/Lhtranslation/Lhtranslation.ts
  var LHTRANSLATION_DOMAIN = "https://lhtranslation.net";
  var LhtranslationInfo = {
    ...
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from LHTranslation",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: LHTRANSLATION_DOMAIN,
  ...
  var Lhtranslation = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${LHTRANSLATION_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${LHTRANSLATION_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${LHTRANSLATION_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${LHTRANSLATION_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${LHTRANSLATION_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
