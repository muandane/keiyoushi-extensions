...
var _Sources = (() => {
  ...
  // src/Battlein5secondsaftermeeting/Battlein5secondsaftermeeting.ts
  var Battlein5secondsaftermeeting_exports = {};
  __export(Battlein5secondsaftermeeting_exports, {
    Battlein5secondsaftermeeting: () => Battlein5secondsaftermeeting,
    Battlein5secondsaftermeetingInfo: () => Battlein5secondsaftermeetingInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Battlein5secondsaftermeeting/Battlein5secondsaftermeetingParser.ts
  ...
  var parseChapters = ($, mangaId) => {
    const chapters = [];
    const chapterElements = $('[class*="chapter"]').toArray();
    for (let i = 0; i < chapterElements.length; i++) {
      const element = chapterElements[i];
      const chapterId = $('[class*="chapter"] a', element).attr("href")?.replace(/\//g, "").split("/").filter(Boolean).pop() || `chapter-${i}`;
      const title = $('[class*="chapter"] a', element).text().trim() || `Chapter ${i + 1}`;
      const chapNum = i + 1;
  ...
  var parseHomeSections = ($, sectionCallback) => {
    const latestItems = [];
    const mangaElements = $('[class*="manga"]').toArray();
    for (const element of mangaElements.slice(0, 20)) {
      const title = $('[class*="manga"] h1', element).first().text().trim();
      const image = $('[class*="manga"] img', element).first().attr("src") || $('[class*="manga"] img', element).first().attr("data-src") || "";
      const link = $('[class*="manga"] a', element).first().attr("href") || "";
  ...
  var parseViewMore = ($, homepageSectionId, metadata) => {
    ...
    for (const element of mangaElements) {
      const title = $('[class*="manga"] h1', element).first().text().trim();
      const image = $('[class*="manga"] img', element).first().attr("src") || "";
      const link = $('[class*="manga"] a', element).first().attr("href") || "";
      const mangaId = link.split("/").filter(Boolean).pop() || "";
      if (title && mangaId) {
  ...
  // src/Battlein5secondsaftermeeting/Battlein5secondsaftermeetingSettings.ts
  ...
  // src/Battlein5secondsaftermeeting/Battlein5secondsaftermeeting.ts
  var BATTLEIN5SECONDSAFTERMEETING_DOMAIN = "https://www.deatte5.com";
  var Battlein5secondsaftermeetingInfo = {
    version: "1.0.0",
    name: "Battle In 5 Seconds After Meeting",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Battle In 5 Seconds After Meeting",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: BATTLEIN5SECONDSAFTERMEETING_DOMAIN,
  ...
  var Battlein5secondsaftermeeting = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${BATTLEIN5SECONDSAFTERMEETING_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${BATTLEIN5SECONDSAFTERMEETING_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${BATTLEIN5SECONDSAFTERMEETING_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${BATTLEIN5SECONDSAFTERMEETING_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: BATTLEIN5SECONDSAFTERMEETING_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${BATTLEIN5SECONDSAFTERMEETING_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${BATTLEIN5SECONDSAFTERMEETING_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Battlein5secondsaftermeeting_exports);
})();
