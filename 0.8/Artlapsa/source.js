...
var _Sources = (() => {
  ...
  // src/Artlapsa/Artlapsa.ts
  var Artlapsa_exports = {};
  __export(Artlapsa_exports, {
    Artlapsa: () => Artlapsa,
    ArtlapsaInfo: () => ArtlapsaInfo
  });
  var import_types = __toESM(require_lib());

  // src/Artlapsa/ArtlapsaParser.ts
  ...
  // src/Artlapsa/ArtlapsaSettings.ts
  ...
  // src/Artlapsa/Artlapsa.ts
  var ARTLAPSA_DOMAIN = "https://artlapsa.com";
  var ArtlapsaInfo = {
    version: "1.0.0",
    name: "Art Lapsa",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Art Lapsa",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: ARTLAPSA_DOMAIN,
  ...
  var Artlapsa = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${ARTLAPSA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${ARTLAPSA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${ARTLAPSA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${ARTLAPSA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Artlapsa_exports);
})();
