...
var _Sources = (() => {
  ...
  // src/Kirascans/Kirascans.ts
  var Kirascans_exports = {};
  __export(Kirascans_exports, {
    Kirascans: () => Kirascans,
    KirascansInfo: () => KirascansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Kirascans/KirascansParser.ts
  ...
  // src/Kirascans/KirascansSettings.ts
  ...
  // src/Kirascans/Kirascans.ts
  var KIRASCANS_DOMAIN = "https://kirascans.com";
  var KirascansInfo = {
    version: "1.0.0",
    name: "Kira Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Kira Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: KIRASCANS_DOMAIN,
  ...
  var Kirascans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${KIRASCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${KIRASCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${KIRASCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${KIRASCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${KIRASCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Kirascans_exports);
})();
