...
var _Sources = (() => {
  ...
  // src/Webtoonscomtranslations/Webtoonscomtranslations.ts
  var Webtoonscomtranslations_exports = {};
  __export(Webtoonscomtranslations_exports, {
    Webtoonscomtranslations: () => Webtoonscomtranslations,
    WebtoonscomtranslationsInfo: () => WebtoonscomtranslationsInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Webtoonscomtranslations/WebtoonscomtranslationsParser.ts
  ...
  // src/Webtoonscomtranslations/WebtoonscomtranslationsSettings.ts
  ...
  // src/Webtoonscomtranslations/Webtoonscomtranslations.ts
  var WEBTOONSCOMTRANSLATIONS_DOMAIN = "https://translate.webtoons.com";
  var WebtoonscomtranslationsInfo = {
    version: "1.0.0",
    name: "Webtoons.com Translations",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Webtoons.com Translations",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: WEBTOONSCOMTRANSLATIONS_DOMAIN,
  ...
  var Webtoonscomtranslations = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${WEBTOONSCOMTRANSLATIONS_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${WEBTOONSCOMTRANSLATIONS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${WEBTOONSCOMTRANSLATIONS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${WEBTOONSCOMTRANSLATIONS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: WEBTOONSCOMTRANSLATIONS_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${WEBTOONSCOMTRANSLATIONS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${WEBTOONSCOMTRANSLATIONS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Webtoonscomtranslations_exports);
})();
