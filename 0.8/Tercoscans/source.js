...
var _Sources = (() => {
  ...
  // src/Tercoscans/Tercoscans.ts
  var Tercoscans_exports = {};
  __export(Tercoscans_exports, {
    Tercoscans: () => Tercoscans,
    TercoscansInfo: () => TercoscansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Tercoscans/TercoscansParser.ts
  ...
  // src/Tercoscans/TercoscansSettings.ts
  ...
  // src/Tercoscans/Tercoscans.ts
  var TERCOSCANS_DOMAIN = "https://tecnocomic1.xyz";
  var TercoscansInfo = {
    version: "1.0.0",
    name: "Terco Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Terco Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: TERCOSCANS_DOMAIN,
  ...
  var Tercoscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${TERCOSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${TERCOSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${TERCOSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${TERCOSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${TERCOSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Tercoscans_exports);
})();
