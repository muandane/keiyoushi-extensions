...
var _Sources = (() => {
  ...
  // src/Keenspottwokinds/Keenspottwokinds.ts
  var Keenspottwokinds_exports = {};
  __export(Keenspottwokinds_exports, {
    Keenspottwokinds: () => Keenspottwokinds,
    KeenspottwokindsInfo: () => KeenspottwokindsInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Keenspottwokinds/KeenspottwokindsParser.ts
  ...
  // src/Keenspottwokinds/KeenspottwokindsSettings.ts
  ...
  // src/Keenspottwokinds/Keenspottwokinds.ts
  var KEENSPOTTWOKINDS_DOMAIN = "https://twokinds.keenspot.com";
  var KeenspottwokindsInfo = {
    version: "1.0.0",
    name: "Keenspot TwoKinds",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Keenspot TwoKinds",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: KEENSPOTTWOKINDS_DOMAIN,
  ...
  var Keenspottwokinds = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${KEENSPOTTWOKINDS_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${KEENSPOTTWOKINDS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${KEENSPOTTWOKINDS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${KEENSPOTTWOKINDS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${KEENSPOTTWOKINDS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${KEENSPOTTWOKINDS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Keenspottwokinds_exports);
})();
