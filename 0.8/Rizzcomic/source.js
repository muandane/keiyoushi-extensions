...
var _Sources = (() => {
  ...
  // src/Rizzcomic/Rizzcomic.ts
  var Rizzcomic_exports = {};
  __export(Rizzcomic_exports, {
    Rizzcomic: () => Rizzcomic,
    RizzcomicInfo: () => RizzcomicInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Rizzcomic/RizzcomicParser.ts
  ...
  // src/Rizzcomic/RizzcomicSettings.ts
  ...
  // src/Rizzcomic/Rizzcomic.ts
  var RIZZCOMIC_DOMAIN = "https://rizzfables.com";
  var RizzcomicInfo = {
    version: "1.0.0",
    name: "Rizz Comic",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Rizz Comic",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: RIZZCOMIC_DOMAIN,
  ...
  var Rizzcomic = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${RIZZCOMIC_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${RIZZCOMIC_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${RIZZCOMIC_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${RIZZCOMIC_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${RIZZCOMIC_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Rizzcomic_exports);
})();
