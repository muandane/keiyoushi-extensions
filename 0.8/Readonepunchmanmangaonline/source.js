...
var _Sources = (() => {
  ...
  // src/Readonepunchmanmangaonline/Readonepunchmanmangaonline.ts
  var Readonepunchmanmangaonline_exports = {};
  __export(Readonepunchmanmangaonline_exports, {
    Readonepunchmanmangaonline: () => Readonepunchmanmangaonline,
    ReadonepunchmanmangaonlineInfo: () => ReadonepunchmanmangaonlineInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Readonepunchmanmangaonline/ReadonepunchmanmangaonlineParser.ts
  ...
  var parseHomeSections = ($, sectionCallback) => {
    const latestItems = [];
    const mangaElements = $(".card").toArray();
    for (const element of mangaElements.slice(0, 20)) {
      const title = $(".card a", element).first().text().trim();
      const image = $(".card img", element).first().attr("src") || $(".card img", element).first().attr("data-src") || "";
      const link = $(".card a", element).first().attr("href") || "";
  ...
  var parseViewMore = ($, homepageSectionId, metadata) => {
    ...
    for (const element of mangaElements) {
      const title = $(".card a", element).first().text().trim();
      const image = $(".card img", element).first().attr("src") || "";
      const link = $(".card a", element).first().attr("href") || "";
      const mangaId = link.split("/").filter(Boolean).pop() || "";
      if (title && mangaId) {
  ...
  // src/Readonepunchmanmangaonline/ReadonepunchmanmangaonlineSettings.ts
  ...
  // src/Readonepunchmanmangaonline/Readonepunchmanmangaonline.ts
  var READONEPUNCHMANMANGAONLINE_DOMAIN = "https://ww6.readopm.com";
  var ReadonepunchmanmangaonlineInfo = {
    version: "1.0.0",
    name: "Read One-Punch Man Manga Online",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Read One-Punch Man Manga Online",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: READONEPUNCHMANMANGAONLINE_DOMAIN,
  ...
  var Readonepunchmanmangaonline = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${READONEPUNCHMANMANGAONLINE_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${READONEPUNCHMANMANGAONLINE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${READONEPUNCHMANMANGAONLINE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${READONEPUNCHMANMANGAONLINE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: READONEPUNCHMANMANGAONLINE_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${READONEPUNCHMANMANGAONLINE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${READONEPUNCHMANMANGAONLINE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Readonepunchmanmangaonline_exports);
})();
