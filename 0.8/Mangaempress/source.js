...
var _Sources = (() => {
  ...
  // src/Mangaempress/Mangaempress.ts
  var Mangaempress_exports = {};
  __export(Mangaempress_exports, {
    Mangaempress: () => Mangaempress,
    MangaempressInfo: () => MangaempressInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Mangaempress/MangaempressParser.ts
  ...
  // src/Mangaempress/MangaempressSettings.ts
  ...
  // src/Mangaempress/Mangaempress.ts
  var MANGAEMPRESS_DOMAIN = "https://mangaempress.com";
  var MangaempressInfo = {
    version: "1.0.0",
    name: "Manga Empress",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Manga Empress",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MANGAEMPRESS_DOMAIN,
  ...
  var Mangaempress = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MANGAEMPRESS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANGAEMPRESS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MANGAEMPRESS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGAEMPRESS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGAEMPRESS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Mangaempress_exports);
})();
