...
var _Sources = (() => {
  ...
  // src/S2mangaio/S2mangaio.ts
  var S2mangaio_exports = {};
  __export(S2mangaio_exports, {
    S2mangaio: () => S2mangaio,
    S2mangaioInfo: () => S2mangaioInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/S2mangaio/S2mangaioParser.ts
  ...
  // src/S2mangaio/S2mangaioSettings.ts
  ...
  // src/S2mangaio/S2mangaio.ts
  var S2MANGAIO_DOMAIN = "https://s2manga.io";
  var S2mangaioInfo = {
    version: "1.0.0",
    name: "S2Manga.io",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from S2Manga.io",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: S2MANGAIO_DOMAIN,
  ...
  var S2mangaio = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${S2MANGAIO_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${S2MANGAIO_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${S2MANGAIO_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${S2MANGAIO_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${S2MANGAIO_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(S2mangaio_exports);
})();
