...
var _Sources = (() => {
  ...
  // src/Rezoscans/Rezoscans.ts
  var Rezoscans_exports = {};
  __export(Rezoscans_exports, {
    Rezoscans: () => Rezoscans,
    RezoscansInfo: () => RezoscansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Rezoscans/RezoscansParser.ts
  ...
  // src/Rezoscans/RezoscansSettings.ts
  ...
  // src/Rezoscans/Rezoscans.ts
  var REZOSCANS_DOMAIN = "https://rezoscan.org";
  var RezoscansInfo = {
    version: "1.0.0",
    name: "Rezo Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Rezo Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: REZOSCANS_DOMAIN,
  ...
  var Rezoscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${REZOSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${REZOSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${REZOSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${REZOSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${REZOSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Rezoscans_exports);
})();
