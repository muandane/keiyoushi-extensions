...
var _Sources = (() => {
  ...
  // src/Grabberzone/Grabberzone.ts
  var Grabberzone_exports = {};
  __export(Grabberzone_exports, {
    Grabberzone: () => Grabberzone,
    GrabberzoneInfo: () => GrabberzoneInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Grabberzone/GrabberzoneParser.ts
  ...
  // src/Grabberzone/GrabberzoneSettings.ts
  ...
  // src/Grabberzone/Grabberzone.ts
  var GRABBERZONE_DOMAIN = "https://grabber.zone";
  var GrabberzoneInfo = {
    version: "1.0.0",
    name: "Grabber Zone",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Grabber Zone",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: GRABBERZONE_DOMAIN,
  ...
  var Grabberzone = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${GRABBERZONE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${GRABBERZONE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${GRABBERZONE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${GRABBERZONE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${GRABBERZONE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Grabberzone_exports);
})();
