...
var _Sources = (() => {
  ...
  // src/Nitroscans/Nitroscans.ts
  var Nitroscans_exports = {};
  __export(Nitroscans_exports, {
    Nitroscans: () => Nitroscans,
    NitroscansInfo: () => NitroscansInfo
  });
  var import_types = __toESM(require_lib());

  // src/Nitroscans/NitroscansParser.ts
  ...
  // src/Nitroscans/NitroscansSettings.ts
  ...
  // src/Nitroscans/Nitroscans.ts
  var NITROSCANS_DOMAIN = "https://nitroscans.net";
  var NitroscansInfo = {
    version: "1.0.0",
    name: "Nitro Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Nitro Scans",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: NITROSCANS_DOMAIN,
  ...
  var Nitroscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${NITROSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${NITROSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${NITROSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${NITROSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${NITROSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Nitroscans_exports);
})();
