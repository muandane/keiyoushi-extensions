...
var _Sources = (() => {
  ...
  // src/Randowiz/Randowiz.ts
  var Randowiz_exports = {};
  __export(Randowiz_exports, {
    Randowiz: () => Randowiz,
    RandowizInfo: () => RandowizInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Randowiz/RandowizParser.ts
  ...
  var parseHomeSections = ($, sectionCallback) => {
    ...
    for (const element of mangaElements.slice(0, 20)) {
      const title = $(".post h3", element).first().text().trim();
  ...
  // src/Randowiz/RandowizSettings.ts
  ...
  // src/Randowiz/Randowiz.ts
  var RANDOWIZ_DOMAIN = "https://randowis.com";
  var RandowizInfo = {
    version: "1.0.0",
    name: "Randowiz",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Randowiz",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: RANDOWIZ_DOMAIN,
  ...
  var Randowiz = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${RANDOWIZ_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${RANDOWIZ_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${RANDOWIZ_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${RANDOWIZ_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Randowiz_exports);
})();
