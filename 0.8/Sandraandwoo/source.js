...
var _Sources = (() => {
  ...
  // src/Sandraandwoo/Sandraandwoo.ts
  var Sandraandwoo_exports = {};
  __export(Sandraandwoo_exports, {
    Sandraandwoo: () => Sandraandwoo,
    SandraandwooInfo: () => SandraandwooInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Sandraandwoo/SandraandwooParser.ts
  ...
  var parseHomeSections = ($, sectionCallback) => {
    ...
    for (const element of mangaElements.slice(0, 20)) {
      const title = $('[class*="comic"] .title, [class*="comic"] .name, [class*="comic"] h3', element).first().text().trim();
  ...
  // src/Sandraandwoo/SandraandwooSettings.ts
  ...
  // src/Sandraandwoo/Sandraandwoo.ts
  var SANDRAANDWOO_DOMAIN = "https://www.sandraandwoo.com";
  var SandraandwooInfo = {
    version: "1.0.0",
    name: "Sandra and Woo",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Sandra and Woo",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: SANDRAANDWOO_DOMAIN,
  ...
  var Sandraandwoo = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${SANDRAANDWOO_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${SANDRAANDWOO_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${SANDRAANDWOO_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${SANDRAANDWOO_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${SANDRAANDWOO_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Sandraandwoo_exports);
})();
