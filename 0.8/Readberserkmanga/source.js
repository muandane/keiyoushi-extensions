...
var _Sources = (() => {
  ...
  // src/Readberserkmanga/Readberserkmanga.ts
  var Readberserkmanga_exports = {};
  __export(Readberserkmanga_exports, {
    Readberserkmanga: () => Readberserkmanga,
    ReadberserkmangaInfo: () => ReadberserkmangaInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Readberserkmanga/ReadberserkmangaParser.ts
  ...
  // src/Readberserkmanga/ReadberserkmangaSettings.ts
  ...
  // src/Readberserkmanga/Readberserkmanga.ts
  var READBERSERKMANGA_DOMAIN = "https://readberserk.com";
  var ReadberserkmangaInfo = {
    version: "1.0.0",
    name: "Read Berserk Manga",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Read Berserk Manga",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: READBERSERKMANGA_DOMAIN,
  ...
  var Readberserkmanga = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${READBERSERKMANGA_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${READBERSERKMANGA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${READBERSERKMANGA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${READBERSERKMANGA_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${READBERSERKMANGA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${READBERSERKMANGA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Readberserkmanga_exports);
})();
