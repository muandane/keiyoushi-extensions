...
var _Sources = (() => {
  ...
  // src/Firstkissmanhua/Firstkissmanhua.ts
  var Firstkissmanhua_exports = {};
  __export(Firstkissmanhua_exports, {
    Firstkissmanhua: () => Firstkissmanhua,
    FirstkissmanhuaInfo: () => FirstkissmanhuaInfo
  });
  var import_types = __toESM(require_lib());

  // src/Firstkissmanhua/FirstkissmanhuaParser.ts
  ...
  // src/Firstkissmanhua/FirstkissmanhuaSettings.ts
  ...
  // src/Firstkissmanhua/Firstkissmanhua.ts
  var FIRSTKISSMANHUA_DOMAIN = "https://1stkissmanhua.net";
  var FirstkissmanhuaInfo = {
    version: "1.0.0",
    name: "First Kiss Manhua",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from First Kiss Manhua",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: FIRSTKISSMANHUA_DOMAIN,
  ...
  var Firstkissmanhua = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${FIRSTKISSMANHUA_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${FIRSTKISSMANHUA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${FIRSTKISSMANHUA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${FIRSTKISSMANHUA_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${FIRSTKISSMANHUA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${FIRSTKISSMANHUA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Firstkissmanhua_exports);
})();
