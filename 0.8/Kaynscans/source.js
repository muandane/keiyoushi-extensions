...
var _Sources = (() => {
  ...
  // src/Kaynscans/Kaynscans.ts
  var Kaynscans_exports = {};
  __export(Kaynscans_exports, {
    Kaynscans: () => Kaynscans,
    KaynscansInfo: () => KaynscansInfo
  });
  var import_types = __toESM(require_lib());

  // src/Kaynscans/KaynscansParser.ts
  ...
  // src/Kaynscans/KaynscansSettings.ts
  ...
  // src/Kaynscans/Kaynscans.ts
  var KAYNSCANS_DOMAIN = "https://kaynscan.com";
  var KaynscansInfo = {
    version: "1.0.0",
    name: "Kayn Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Kayn Scans",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: KAYNSCANS_DOMAIN,
  ...
  var Kaynscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${KAYNSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${KAYNSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${KAYNSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${KAYNSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${KAYNSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Kaynscans_exports);
})();
