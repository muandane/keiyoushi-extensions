...
var _Sources = (() => {
  ...
  // src/Platinumcrown/Platinumcrown.ts
  var Platinumcrown_exports = {};
  __export(Platinumcrown_exports, {
    Platinumcrown: () => Platinumcrown,
    PlatinumcrownInfo: () => PlatinumcrownInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Platinumcrown/PlatinumcrownParser.ts
  ...
  // src/Platinumcrown/PlatinumcrownSettings.ts
  ...
  // src/Platinumcrown/Platinumcrown.ts
  var PLATINUMCROWN_DOMAIN = "https://platinumscans.com";
  var PlatinumcrownInfo = {
    version: "1.0.0",
    name: "Platinum Crown",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Platinum Crown",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: PLATINUMCROWN_DOMAIN,
  ...
  var Platinumcrown = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${PLATINUMCROWN_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${PLATINUMCROWN_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${PLATINUMCROWN_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${PLATINUMCROWN_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${PLATINUMCROWN_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Platinumcrown_exports);
})();
