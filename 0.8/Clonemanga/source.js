...
var _Sources = (() => {
  ...
  // src/Clonemanga/Clonemanga.ts
  var Clonemanga_exports = {};
  __export(Clonemanga_exports, {
    Clonemanga: () => Clonemanga,
    ClonemangaInfo: () => ClonemangaInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Clonemanga/ClonemangaParser.ts
  ...
  // src/Clonemanga/ClonemangaSettings.ts
  ...
  // src/Clonemanga/Clonemanga.ts
  var CLONEMANGA_DOMAIN = "https://manga.clone-army.org";
  var ClonemangaInfo = {
    version: "1.0.0",
    name: "Clone Manga",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Clone Manga",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: CLONEMANGA_DOMAIN,
  ...
  var Clonemanga = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${CLONEMANGA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${CLONEMANGA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${CLONEMANGA_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${CLONEMANGA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${CLONEMANGA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Clonemanga_exports);
})();
