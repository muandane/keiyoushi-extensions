...
var _Sources = (() => {
  ...
  // src/Mangademon/MangademonSettings.ts
  ...
  // src/Mangademon/Mangademon.ts
  var MANGADEMON_DOMAIN = "https://demonicscans.org";
  var MangademonInfo = {
    version: "1.0.0",
    name: "Manga Demon",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Manga Demon",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MANGADEMON_DOMAIN,
  ...
  var Mangademon = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MANGADEMON_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANGADEMON_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MANGADEMON_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGADEMON_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGADEMON_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
