...
var _Sources = (() => {
  ...
  // src/Mangaleveling/Mangaleveling.ts
  var Mangaleveling_exports = {};
  __export(Mangaleveling_exports, {
    Mangaleveling: () => Mangaleveling,
    MangalevelingInfo: () => MangalevelingInfo
  });
  var import_types = __toESM(require_lib());

  // src/Mangaleveling/MangalevelingParser.ts
  ...
  // src/Mangaleveling/MangalevelingSettings.ts
  ...
  // src/Mangaleveling/Mangaleveling.ts
  var MANGALEVELING_DOMAIN = "https://mangaleveling.com";
  var MangalevelingInfo = {
    version: "1.0.0",
    name: "Manga Leveling",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Manga Leveling",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: MANGALEVELING_DOMAIN,
  ...
  var Mangaleveling = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MANGALEVELING_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANGALEVELING_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MANGALEVELING_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGALEVELING_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGALEVELING_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Mangaleveling_exports);
})();
