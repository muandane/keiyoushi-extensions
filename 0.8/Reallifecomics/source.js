...
var _Sources = (() => {
  ...
  // src/Reallifecomics/Reallifecomics.ts
  var Reallifecomics_exports = {};
  __export(Reallifecomics_exports, {
    Reallifecomics: () => Reallifecomics,
    ReallifecomicsInfo: () => ReallifecomicsInfo
  });
  var import_types = __toESM(require_lib());

  // src/Reallifecomics/ReallifecomicsParser.ts
  ...
  // src/Reallifecomics/ReallifecomicsSettings.ts
  ...
  // src/Reallifecomics/Reallifecomics.ts
  var REALLIFECOMICS_DOMAIN = "https://reallifecomics.com";
  var ReallifecomicsInfo = {
    version: "1.0.0",
    name: "Real Life Comics",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Real Life Comics",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: REALLIFECOMICS_DOMAIN,
  ...
  var Reallifecomics = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${REALLIFECOMICS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${REALLIFECOMICS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${REALLIFECOMICS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${REALLIFECOMICS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${REALLIFECOMICS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Reallifecomics_exports);
})();
