...
var _Sources = (() => {
  ...
  // src/Loadingartist/Loadingartist.ts
  var Loadingartist_exports = {};
  __export(Loadingartist_exports, {
    Loadingartist: () => Loadingartist,
    LoadingartistInfo: () => LoadingartistInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Loadingartist/LoadingartistParser.ts
  ...
  var parseHomeSections = ($, sectionCallback) => {
    const latestItems = [];
    const mangaElements = $("article").toArray();
    for (const element of mangaElements.slice(0, 20)) {
      const title = $("article h1", element).first().text().trim();
      const image = $("article img", element).first().attr("src") || $("article img", element).first().attr("data-src") || "";
      const link = $("article a", element).first().attr("href") || "";
  ...
  var parseViewMore = ($, homepageSectionId, metadata) => {
    ...
    for (const element of mangaElements) {
      const title = $("article h1", element).first().text().trim();
      const image = $("article img", element).first().attr("src") || "";
      const link = $("article a", element).first().attr("href") || "";
      const mangaId = link.split("/").filter(Boolean).pop() || "";
      if (title && mangaId) {
  ...
  // src/Loadingartist/LoadingartistSettings.ts
  ...
  // src/Loadingartist/Loadingartist.ts
  var LOADINGARTIST_DOMAIN = "https://loadingartist.com";
  var LoadingartistInfo = {
    version: "1.0.0",
    name: "Loading Artist",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Loading Artist",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: LOADINGARTIST_DOMAIN,
  ...
  var Loadingartist = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${LOADINGARTIST_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${LOADINGARTIST_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${LOADINGARTIST_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${LOADINGARTIST_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${LOADINGARTIST_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Loadingartist_exports);
})();
