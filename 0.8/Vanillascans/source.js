...
var _Sources = (() => {
  ...
  // src/Vanillascans/Vanillascans.ts
  var Vanillascans_exports = {};
  __export(Vanillascans_exports, {
    Vanillascans: () => Vanillascans,
    VanillascansInfo: () => VanillascansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Vanillascans/VanillascansParser.ts
  ...
  // src/Vanillascans/VanillascansSettings.ts
  ...
  // src/Vanillascans/Vanillascans.ts
  var VANILLASCANS_DOMAIN = "https://vanillascans.org";
  var VanillascansInfo = {
    version: "1.0.0",
    name: "Vanilla Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Vanilla Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: VANILLASCANS_DOMAIN,
  ...
  var Vanillascans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${VANILLASCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${VANILLASCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${VANILLASCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${VANILLASCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${VANILLASCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Vanillascans_exports);
})();
