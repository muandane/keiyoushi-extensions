...
var _Sources = (() => {
  ...
  // src/Komga2/Komga2.ts
  var Komga2_exports = {};
  __export(Komga2_exports, {
    Komga2: () => Komga2,
    Komga2Info: () => Komga2Info
  });
  var import_types = __toESM(require_lib());

  // src/Komga2/Komga2Parser.ts
  ...
  var Komga2Info = {
    ...
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Komga (2)",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: KOMGA2_DOMAIN,
  ...
  var Komga2 = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${KOMGA2_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${KOMGA2_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${KOMGA2_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${KOMGA2_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Komga2_exports);
})();
