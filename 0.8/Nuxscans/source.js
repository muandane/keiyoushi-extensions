...
var _Sources = (() => {
  ...
  // src/Nuxscans/Nuxscans.ts
  var Nuxscans_exports = {};
  __export(Nuxscans_exports, {
    Nuxscans: () => Nuxscans,
    NuxscansInfo: () => NuxscansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Nuxscans/NuxscansParser.ts
  ...
  // src/Nuxscans/NuxscansSettings.ts
  ...
  // src/Nuxscans/Nuxscans.ts
  var NUXSCANS_DOMAIN = "https://nuxscans.blogspot.com";
  var NuxscansInfo = {
    version: "1.0.0",
    name: "Nux Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Nux Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: NUXSCANS_DOMAIN,
  ...
  var Nuxscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${NUXSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${NUXSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${NUXSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${NUXSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Nuxscans_exports);
})();
