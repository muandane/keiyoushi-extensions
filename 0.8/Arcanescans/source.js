...
var _Sources = (() => {
  ...
  // src/Arcanescans/Arcanescans.ts
  var Arcanescans_exports = {};
  __export(Arcanescans_exports, {
    Arcanescans: () => Arcanescans,
    ArcanescansInfo: () => ArcanescansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Arcanescans/ArcanescansParser.ts
  ...
  // src/Arcanescans/ArcanescansSettings.ts
  ...
  // src/Arcanescans/Arcanescans.ts
  var ARCANESCANS_DOMAIN = "https://arcanescans.com";
  var ArcanescansInfo = {
    version: "1.0.0",
    name: "Arcanescans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Arcanescans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: ARCANESCANS_DOMAIN,
  ...
  var Arcanescans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${ARCANESCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${ARCANESCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${ARCANESCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${ARCANESCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${ARCANESCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Arcanescans_exports);
})();
