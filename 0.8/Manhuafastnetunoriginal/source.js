...
var _Sources = (() => {
  ...
  // src/Manhuafastnetunoriginal/Manhuafastnetunoriginal.ts
  var Manhuafastnetunoriginal_exports = {};
  __export(Manhuafastnetunoriginal_exports, {
    Manhuafastnetunoriginal: () => Manhuafastnetunoriginal,
    ManhuafastnetunoriginalInfo: () => ManhuafastnetunoriginalInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Manhuafastnetunoriginal/ManhuafastnetunoriginalParser.ts
  ...
  var parseHomeSections = ($, sectionCallback) => {
    ...
    for (const element of mangaElements.slice(0, 20)) {
      const title = $('[class*="manga"] a', element).first().text().trim();
  ...
  // src/Manhuafastnetunoriginal/ManhuafastnetunoriginalSettings.ts
  ...
  // src/Manhuafastnetunoriginal/Manhuafastnetunoriginal.ts
  var MANHUAFASTNETUNORIGINAL_DOMAIN = "https://manhuafast.net";
  var ManhuafastnetunoriginalInfo = {
    version: "1.0.0",
    name: "ManhuaFast.net (unoriginal)",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from ManhuaFast.net (unoriginal)",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MANHUAFASTNETUNORIGINAL_DOMAIN,
  ...
  var Manhuafastnetunoriginal = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${MANHUAFASTNETUNORIGINAL_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${MANHUAFASTNETUNORIGINAL_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANHUAFASTNETUNORIGINAL_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MANHUAFASTNETUNORIGINAL_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: MANHUAFASTNETUNORIGINAL_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANHUAFASTNETUNORIGINAL_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANHUAFASTNETUNORIGINAL_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Manhuafastnetunoriginal_exports);
})();
