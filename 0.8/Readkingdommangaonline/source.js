...
var _Sources = (() => {
  ...
  // src/Readkingdommangaonline/Readkingdommangaonline.ts
  var Readkingdommangaonline_exports = {};
  __export(Readkingdommangaonline_exports, {
    Readkingdommangaonline: () => Readkingdommangaonline,
    ReadkingdommangaonlineInfo: () => ReadkingdommangaonlineInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Readkingdommangaonline/ReadkingdommangaonlineParser.ts
  ...
  // src/Readkingdommangaonline/ReadkingdommangaonlineSettings.ts
  ...
  // src/Readkingdommangaonline/Readkingdommangaonline.ts
  var READKINGDOMMANGAONLINE_DOMAIN = "https://ww5.readkingdom.com";
  var ReadkingdommangaonlineInfo = {
    version: "1.0.0",
    name: "Read Kingdom Manga Online",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Read Kingdom Manga Online",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: READKINGDOMMANGAONLINE_DOMAIN,
  ...
  var Readkingdommangaonline = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${READKINGDOMMANGAONLINE_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${READKINGDOMMANGAONLINE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${READKINGDOMMANGAONLINE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${READKINGDOMMANGAONLINE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: READKINGDOMMANGAONLINE_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${READKINGDOMMANGAONLINE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${READKINGDOMMANGAONLINE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Readkingdommangaonline_exports);
})();
