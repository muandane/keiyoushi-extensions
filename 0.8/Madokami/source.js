...
var _Sources = (() => {
  ...
  // src/Madokami/Madokami.ts
  var Madokami_exports = {};
  __export(Madokami_exports, {
    Madokami: () => Madokami,
    MadokamiInfo: () => MadokamiInfo
  });
  var import_types = __toESM(require_lib());

  // src/Madokami/MadokamiParser.ts
  ...
  // src/Madokami/MadokamiSettings.ts
  ...
  // src/Madokami/Madokami.ts
  var MADOKAMI_DOMAIN = "https://manga.madokami.al";
  var MadokamiInfo = {
    version: "1.0.0",
    name: "Madokami",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Madokami",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: MADOKAMI_DOMAIN,
  ...
  var Madokami = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MADOKAMI_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MADOKAMI_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MADOKAMI_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MADOKAMI_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Madokami_exports);
})();
