...
var _Sources = (() => {
  ...
  // src/Supermega/Supermega.ts
  var Supermega_exports = {};
  __export(Supermega_exports, {
    Supermega: () => Supermega,
    SupermegaInfo: () => SupermegaInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Supermega/SupermegaParser.ts
  ...
  // src/Supermega/SupermegaSettings.ts
  ...
  // src/Supermega/Supermega.ts
  var SUPERMEGA_DOMAIN = "https://www.supermegacomics.com";
  var SupermegaInfo = {
    version: "1.0.0",
    name: "SUPER MEGA",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from SUPER MEGA",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: SUPERMEGA_DOMAIN,
  ...
  var Supermega = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${SUPERMEGA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${SUPERMEGA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${SUPERMEGA_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${SUPERMEGA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${SUPERMEGA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Supermega_exports);
})();
