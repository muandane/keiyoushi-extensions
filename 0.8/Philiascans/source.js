...
var _Sources = (() => {
  ...
  // src/Philiascans/Philiascans.ts
  var Philiascans_exports = {};
  __export(Philiascans_exports, {
    Philiascans: () => Philiascans,
    PhiliascansInfo: () => PhiliascansInfo
  });
  var import_types = __toESM(require_lib());

  // src/Philiascans/PhiliascansParser.ts
  ...
  // src/Philiascans/PhiliascansSettings.ts
  ...
  // src/Philiascans/Philiascans.ts
  var PHILIASCANS_DOMAIN = "https://philiascans.org";
  var PhiliascansInfo = {
    version: "1.0.0",
    name: "Philia Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Philia Scans",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: PHILIASCANS_DOMAIN,
  ...
  var Philiascans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${PHILIASCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${PHILIASCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${PHILIASCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${PHILIASCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${PHILIASCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Philiascans_exports);
})();
