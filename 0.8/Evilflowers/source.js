...
var _Sources = (() => {
  ...
  // src/Evilflowers/Evilflowers.ts
  var Evilflowers_exports = {};
  __export(Evilflowers_exports, {
    Evilflowers: () => Evilflowers,
    EvilflowersInfo: () => EvilflowersInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Evilflowers/EvilflowersParser.ts
  ...
  // src/Evilflowers/EvilflowersSettings.ts
  ...
  // src/Evilflowers/Evilflowers.ts
  var EVILFLOWERS_DOMAIN = "https://evilflowers.com";
  var EvilflowersInfo = {
    version: "1.0.0",
    name: "Evil Flowers",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Evil Flowers",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: EVILFLOWERS_DOMAIN,
  ...
  var Evilflowers = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${EVILFLOWERS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${EVILFLOWERS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${EVILFLOWERS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${EVILFLOWERS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${EVILFLOWERS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Evilflowers_exports);
})();
