...
var _Sources = (() => {
  ...
  // src/Vortexscans/Vortexscans.ts
  var Vortexscans_exports = {};
  __export(Vortexscans_exports, {
    Vortexscans: () => Vortexscans,
    VortexscansInfo: () => VortexscansInfo
  });
  var import_types = __toESM(require_lib());

  // src/Vortexscans/VortexscansParser.ts
  ...
  // src/Vortexscans/VortexscansSettings.ts
  ...
  // src/Vortexscans/Vortexscans.ts
  var VORTEXSCANS_DOMAIN = "https://vortexscans.org";
  var VortexscansInfo = {
    version: "1.0.0",
    name: "Vortex Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Vortex Scans",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: VORTEXSCANS_DOMAIN,
  ...
  var Vortexscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${VORTEXSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${VORTEXSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${VORTEXSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${VORTEXSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${VORTEXSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Vortexscans_exports);
})();
