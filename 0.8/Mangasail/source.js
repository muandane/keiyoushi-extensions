...
var _Sources = (() => {
  ...
  // src/Mangasail/Mangasail.ts
  var Mangasail_exports = {};
  __export(Mangasail_exports, {
    Mangasail: () => Mangasail,
    MangasailInfo: () => MangasailInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Mangasail/MangasailParser.ts
  ...
  // src/Mangasail/MangasailSettings.ts
  ...
  // src/Mangasail/Mangasail.ts
  var MANGASAIL_DOMAIN = "https://www.sailmg.com";
  var MangasailInfo = {
    version: "1.0.0",
    name: "Mangasail",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Mangasail",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MANGASAIL_DOMAIN,
  ...
  var Mangasail = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MANGASAIL_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANGASAIL_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MANGASAIL_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGASAIL_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGASAIL_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Mangasail_exports);
})();
