...
var _Sources = (() => {
  ...
  // src/Genztoons/Genztoons.ts
  var Genztoons_exports = {};
  __export(Genztoons_exports, {
    Genztoons: () => Genztoons,
    GenztoonsInfo: () => GenztoonsInfo
  });
  var import_types = __toESM(require_lib());

  // src/Genztoons/GenztoonsParser.ts
  ...
  // src/Genztoons/GenztoonsSettings.ts
  ...
  // src/Genztoons/Genztoons.ts
  var GENZTOONS_DOMAIN = "https://genzupdates.com";
  var GenztoonsInfo = {
    version: "1.0.0",
    name: "Genz Toons",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Genz Toons",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: GENZTOONS_DOMAIN,
  ...
  var Genztoons = class {
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${GENZTOONS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${GENZTOONS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${GENZTOONS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Genztoons_exports);
})();
