...
var _Sources = (() => {
  ...
  // src/Saturdaymorningbreakfastcomics/Saturdaymorningbreakfastcomics.ts
  var Saturdaymorningbreakfastcomics_exports = {};
  __export(Saturdaymorningbreakfastcomics_exports, {
    Saturdaymorningbreakfastcomics: () => Saturdaymorningbreakfastcomics,
    SaturdaymorningbreakfastcomicsInfo: () => SaturdaymorningbreakfastcomicsInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Saturdaymorningbreakfastcomics/SaturdaymorningbreakfastcomicsParser.ts
  ...
  // src/Saturdaymorningbreakfastcomics/SaturdaymorningbreakfastcomicsSettings.ts
  ...
  // src/Saturdaymorningbreakfastcomics/Saturdaymorningbreakfastcomics.ts
  var SATURDAYMORNINGBREAKFASTCOMICS_DOMAIN = "https://smbc-comics.com";
  var SaturdaymorningbreakfastcomicsInfo = {
    version: "1.0.0",
    name: "Saturday Morning Breakfast Comics",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Saturday Morning Breakfast Comics",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: SATURDAYMORNINGBREAKFASTCOMICS_DOMAIN,
  ...
  var Saturdaymorningbreakfastcomics = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${SATURDAYMORNINGBREAKFASTCOMICS_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${SATURDAYMORNINGBREAKFASTCOMICS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${SATURDAYMORNINGBREAKFASTCOMICS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${SATURDAYMORNINGBREAKFASTCOMICS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: SATURDAYMORNINGBREAKFASTCOMICS_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${SATURDAYMORNINGBREAKFASTCOMICS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${SATURDAYMORNINGBREAKFASTCOMICS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Saturdaymorningbreakfastcomics_exports);
})();
