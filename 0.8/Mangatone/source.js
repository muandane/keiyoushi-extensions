...
var _Sources = (() => {
  ...
  // src/Mangatone/Mangatone.ts
  var Mangatone_exports = {};
  __export(Mangatone_exports, {
    Mangatone: () => Mangatone,
    MangatoneInfo: () => MangatoneInfo
  });
  var import_types = __toESM(require_lib());

  // src/Mangatone/MangatoneParser.ts
  ...
  // src/Mangatone/MangatoneSettings.ts
  ...
  // src/Mangatone/Mangatone.ts
  var MANGATONE_DOMAIN = "https://mangatone.com";
  var MangatoneInfo = {
    version: "1.0.0",
    name: "MangaTone",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from MangaTone",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: MANGATONE_DOMAIN,
  ...
  var Mangatone = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${MANGATONE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANGATONE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MANGATONE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGATONE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGATONE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Mangatone_exports);
})();
