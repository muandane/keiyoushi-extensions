...
var _Sources = (() => {
  ...
  // src/Shootingstarscans/Shootingstarscans.ts
  var Shootingstarscans_exports = {};
  __export(Shootingstarscans_exports, {
    Shootingstarscans: () => Shootingstarscans,
    ShootingstarscansInfo: () => ShootingstarscansInfo
  });
  var import_types = __toESM(require_lib());

  // src/Shootingstarscans/ShootingstarscansParser.ts
  ...
  // src/Shootingstarscans/ShootingstarscansSettings.ts
  ...
  // src/Shootingstarscans/Shootingstarscans.ts
  var SHOOTINGSTARSCANS_DOMAIN = "https://shootingstarscans.com";
  var ShootingstarscansInfo = {
    version: "1.0.0",
    name: "Shooting Star Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Shooting Star Scans",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: SHOOTINGSTARSCANS_DOMAIN,
  ...
  var Shootingstarscans = class {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${SHOOTINGSTARSCANS_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${SHOOTINGSTARSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${SHOOTINGSTARSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${SHOOTINGSTARSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${SHOOTINGSTARSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${SHOOTINGSTARSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Shootingstarscans_exports);
})();
