...
var _Sources = (() => {
  ...
  // src/Xoxocomics/Xoxocomics.ts
  var Xoxocomics_exports = {};
  __export(Xoxocomics_exports, {
    Xoxocomics: () => Xoxocomics,
    XoxocomicsInfo: () => XoxocomicsInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Xoxocomics/XoxocomicsParser.ts
  ...
  // src/Xoxocomics/XoxocomicsSettings.ts
  ...
  // src/Xoxocomics/Xoxocomics.ts
  var XOXOCOMICS_DOMAIN = "https://xoxocomic.com";
  var XoxocomicsInfo = {
    version: "1.0.0",
    name: "XOXO Comics",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from XOXO Comics",
  ...
  var Xoxocomics = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${XOXOCOMICS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${XOXOCOMICS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${XOXOCOMICS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${XOXOCOMICS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${XOXOCOMICS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Xoxocomics_exports);
})();
