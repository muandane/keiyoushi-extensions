...
var _Sources = (() => {
  ...
  // src/Readbokunoheroacademiamyheroacademiamanga/Readbokunoheroacademiamyheroacademiamanga.ts
  var Readbokunoheroacademiamyheroacademiamanga_exports = {};
  __export(Readbokunoheroacademiamyheroacademiamanga_exports, {
    Readbokunoheroacademiamyheroacademiamanga: () => Readbokunoheroacademiamyheroacademiamanga,
    ReadbokunoheroacademiamyheroacademiamangaInfo: () => ReadbokunoheroacademiamyheroacademiamangaInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Readbokunoheroacademiamyheroacademiamanga/ReadbokunoheroacademiamyheroacademiamangaParser.ts
  ...
  // src/Readbokunoheroacademiamyheroacademiamanga/ReadbokunoheroacademiamyheroacademiamangaSettings.ts
  ...
  // src/Readbokunoheroacademiamyheroacademiamanga/Readbokunoheroacademiamyheroacademiamanga.ts
  var READBOKUNOHEROACADEMIAMYHEROACADEMIAMANGA_DOMAIN = "https://ww10.readmha.com";
  var ReadbokunoheroacademiamyheroacademiamangaInfo = {
    version: "1.0.0",
    name: "Read Boku no Hero Academia My Hero Academia Manga",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Read Boku no Hero Academia My Hero Academia Manga",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: READBOKUNOHEROACADEMIAMYHEROACADEMIAMANGA_DOMAIN,
  ...
  var Readbokunoheroacademiamyheroacademiamanga = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${READBOKUNOHEROACADEMIAMYHEROACADEMIAMANGA_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${READBOKUNOHEROACADEMIAMYHEROACADEMIAMANGA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${READBOKUNOHEROACADEMIAMYHEROACADEMIAMANGA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${READBOKUNOHEROACADEMIAMYHEROACADEMIAMANGA_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: READBOKUNOHEROACADEMIAMYHEROACADEMIAMANGA_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${READBOKUNOHEROACADEMIAMYHEROACADEMIAMANGA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${READBOKUNOHEROACADEMIAMYHEROACADEMIAMANGA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Readbokunoheroacademiamyheroacademiamanga_exports);
})();
