...
var _Sources = (() => {
  ...
  // src/Tritiniascans/Tritiniascans.ts
  var Tritiniascans_exports = {};
  __export(Tritiniascans_exports, {
    Tritiniascans: () => Tritiniascans,
    TritiniascansInfo: () => TritiniascansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Tritiniascans/TritiniascansParser.ts
  ...
  // src/Tritiniascans/TritiniascansSettings.ts
  ...
  // src/Tritiniascans/Tritiniascans.ts
  var TRITINIASCANS_DOMAIN = "https://tritinia.org";
  var TritiniascansInfo = {
    version: "1.0.0",
    name: "TritiniaScans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from TritiniaScans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: TRITINIASCANS_DOMAIN,
  ...
  var Tritiniascans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${TRITINIASCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${TRITINIASCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${TRITINIASCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${TRITINIASCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${TRITINIASCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Tritiniascans_exports);
})();
