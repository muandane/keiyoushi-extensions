...
var _Sources = (() => {
  ...
  // src/Utoon/Utoon.ts
  var UTOON_DOMAIN = "https://utoon.net";
  var UtoonInfo = {
    version: "1.0.0",
    name: "Utoon",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Utoon",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: UTOON_DOMAIN,
  ...
  var Utoon = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${UTOON_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${UTOON_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${UTOON_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${UTOON_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
