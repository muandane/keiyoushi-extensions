...
var _Sources = (() => {
  ...
  // src/Komga3/Komga3.ts
  var Komga3_exports = {};
  __export(Komga3_exports, {
    Komga3: () => Komga3,
    Komga3Info: () => Komga3Info
  });
  var import_types = __toESM(require_lib());

  // src/Komga3/Komga3Parser.ts
  ...
  var Komga3Info = {
    ...
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Komga (3)",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: KOMGA3_DOMAIN,
  ...
  var Komga3 = class {
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${KOMGA3_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${KOMGA3_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Komga3_exports);
})();
