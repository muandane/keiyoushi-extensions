...
var _Sources = (() => {
  ...
  // src/Kmanga/Kmanga.ts
  var Kmanga_exports = {};
  __export(Kmanga_exports, {
    Kmanga: () => Kmanga,
    KmangaInfo: () => KmangaInfo
  });
  var import_types = __toESM(require_lib());

  // src/Kmanga/KmangaParser.ts
  ...
  // src/Kmanga/Kmanga.ts
  var KMANGA_DOMAIN = "https://kmanga.kodansha.com";
  var KmangaInfo = {
    version: "1.0.0",
    name: "K Manga",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from K Manga",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: KMANGA_DOMAIN,
  ...
  var Kmanga = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${KMANGA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${KMANGA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${KMANGA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${KMANGA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Kmanga_exports);
})();
