...
var _Sources = (() => {
  ...
  // src/Rokaricomics/RokaricomicsParser.ts
  ...
  // src/Rokaricomics/RokaricomicsSettings.ts
  ...
  // src/Rokaricomics/Rokaricomics.ts
  var ROKARICOMICS_DOMAIN = "https://rokaricomics.com";
  var RokaricomicsInfo = {
    version: "1.0.0",
    name: "RokariComics",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from RokariComics",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: ROKARICOMICS_DOMAIN,
  ...
  var Rokaricomics = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${ROKARICOMICS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${ROKARICOMICS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${ROKARICOMICS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${ROKARICOMICS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${ROKARICOMICS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
