...
var _Sources = (() => {
  ...
  // src/Kewnscans/Kewnscans.ts
  var Kewnscans_exports = {};
  __export(Kewnscans_exports, {
    Kewnscans: () => Kewnscans,
    KewnscansInfo: () => KewnscansInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Kewnscans/KewnscansParser.ts
  ...
  // src/Kewnscans/KewnscansSettings.ts
  ...
  // src/Kewnscans/Kewnscans.ts
  var KEWNSCANS_DOMAIN = "https://kewnscans.org";
  var KewnscansInfo = {
    version: "1.0.0",
    name: "Kewn Scans",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from Kewn Scans",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: KEWNSCANS_DOMAIN,
  ...
  var Kewnscans = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${KEWNSCANS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${KEWNSCANS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${KEWNSCANS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${KEWNSCANS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${KEWNSCANS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Kewnscans_exports);
})();
