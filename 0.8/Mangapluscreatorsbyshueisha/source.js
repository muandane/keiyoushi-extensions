...
var _Sources = (() => {
  ...
  // src/Mangapluscreatorsbyshueisha/Mangapluscreatorsbyshueisha.ts
  var Mangapluscreatorsbyshueisha_exports = {};
  __export(Mangapluscreatorsbyshueisha_exports, {
    Mangapluscreatorsbyshueisha: () => Mangapluscreatorsbyshueisha,
    MangapluscreatorsbyshueishaInfo: () => MangapluscreatorsbyshueishaInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Mangapluscreatorsbyshueisha/MangapluscreatorsbyshueishaParser.ts
  ...
  var parseChapters = ($, mangaId) => {
    const chapters = [];
    const chapterElements = $("ul li").toArray();
    for (let i = 0; i < chapterElements.length; i++) {
      const element = chapterElements[i];
      const chapterId = $("ul li a", element).attr("href")?.replace(/\//g, "").split("/").filter(Boolean).pop() || `chapter-${i}`;
      const title = $("ul li a", element).text().trim() || `Chapter ${i + 1}`;
      const chapNum = i + 1;
  ...
  var parseHomeSections = ($, sectionCallback) => {
    ...
    for (const element of mangaElements.slice(0, 20)) {
      const title = $('[class*="manga"] h2', element).first().text().trim();
  ...
  // src/Mangapluscreatorsbyshueisha/MangapluscreatorsbyshueishaSettings.ts
  ...
  // src/Mangapluscreatorsbyshueisha/Mangapluscreatorsbyshueisha.ts
  var MANGAPLUSCREATORSBYSHUEISHA_DOMAIN = "https://mangaplus-creators.jp";
  var MangapluscreatorsbyshueishaInfo = {
    version: "1.0.0",
    name: "MANGA Plus Creators by SHUEISHA",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from MANGA Plus Creators by SHUEISHA",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: MANGAPLUSCREATORSBYSHUEISHA_DOMAIN,
  ...
  var Mangapluscreatorsbyshueisha = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              ...{
                "referer": `${MANGAPLUSCREATORSBYSHUEISHA_DOMAIN}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${MANGAPLUSCREATORSBYSHUEISHA_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MANGAPLUSCREATORSBYSHUEISHA_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${MANGAPLUSCREATORSBYSHUEISHA_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getHomePageSections(sectionCallback) {
      const request = App.createRequest({
        url: MANGAPLUSCREATORSBYSHUEISHA_DOMAIN,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGAPLUSCREATORSBYSHUEISHA_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${MANGAPLUSCREATORSBYSHUEISHA_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Mangapluscreatorsbyshueisha_exports);
})();
