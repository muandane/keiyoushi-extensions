...
var _Sources = (() => {
  ...
  // src/Elgoonishshive/Elgoonishshive.ts
  var Elgoonishshive_exports = {};
  __export(Elgoonishshive_exports, {
    Elgoonishshive: () => Elgoonishshive,
    ElgoonishshiveInfo: () => ElgoonishshiveInfo
  });
  var import_types = __toESM(require_lib());

  // src/Elgoonishshive/ElgoonishshiveParser.ts
  ...
  // src/Elgoonishshive/ElgoonishshiveSettings.ts
  ...
  // src/Elgoonishshive/Elgoonishshive.ts
  var ELGOONISHSHIVE_DOMAIN = "https://www.egscomics.com";
  var ElgoonishshiveInfo = {
    version: "1.0.0",
    name: "El Goonish Shive",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from El Goonish Shive",
    contentRating: import_types.ContentRating.EVERYONE,
    websiteBaseURL: ELGOONISHSHIVE_DOMAIN,
  ...
  var Elgoonishshive = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${ELGOONISHSHIVE_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${ELGOONISHSHIVE_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${ELGOONISHSHIVE_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${ELGOONISHSHIVE_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${ELGOONISHSHIVE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Elgoonishshive_exports);
})();
