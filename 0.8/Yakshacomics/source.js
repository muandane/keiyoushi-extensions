...
var _Sources = (() => {
  ...
  // src/Yakshacomics/Yakshacomics.ts
  var Yakshacomics_exports = {};
  __export(Yakshacomics_exports, {
    Yakshacomics: () => Yakshacomics,
    YakshacomicsInfo: () => YakshacomicsInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Yakshacomics/YakshacomicsParser.ts
  ...
  // src/Yakshacomics/YakshacomicsSettings.ts
  ...
  // src/Yakshacomics/Yakshacomics.ts
  var YAKSHACOMICS_DOMAIN = "https://yakshacomics.com";
  var YakshacomicsInfo = {
    version: "1.0.0",
    name: "YakshaComics",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from YakshaComics",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: YAKSHACOMICS_DOMAIN,
  ...
  var Yakshacomics = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${YAKSHACOMICS_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${YAKSHACOMICS_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${YAKSHACOMICS_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${YAKSHACOMICS_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${YAKSHACOMICS_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Yakshacomics_exports);
})();
