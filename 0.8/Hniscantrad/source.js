...
var _Sources = (() => {
  ...
  // src/Hniscantrad/Hniscantrad.ts
  var Hniscantrad_exports = {};
  __export(Hniscantrad_exports, {
    Hniscantrad: () => Hniscantrad,
    HniscantradInfo: () => HniscantradInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Hniscantrad/HniscantradParser.ts
  ...
  // src/Hniscantrad/HniscantradSettings.ts
  ...
  // src/Hniscantrad/Hniscantrad.ts
  var HNISCANTRAD_DOMAIN = "https://hni-scantrad.net";
  var HniscantradInfo = {
    version: "1.0.0",
    name: "HNI-Scantrad",
    icon: "icon.png",
    author: "Generated",
    authorWebsite: "",
    description: "Extension that pulls manga from HNI-Scantrad",
    contentRating: import_types2.ContentRating.EVERYONE,
    websiteBaseURL: HNISCANTRAD_DOMAIN,
  ...
  var Hniscantrad = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${HNISCANTRAD_DOMAIN}/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${HNISCANTRAD_DOMAIN}/${mangaId}`,
        method: "GET"
      });
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${HNISCANTRAD_DOMAIN}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const request = App.createRequest({
        url: `${HNISCANTRAD_DOMAIN}/${homepageSectionId}?page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
    ...
    async getSearchResults(query, metadata) {
      ...
      const request = App.createRequest({
        url: `${HNISCANTRAD_DOMAIN}/search?q=${searchQuery}&page=${page}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(Hniscantrad_exports);
})();
