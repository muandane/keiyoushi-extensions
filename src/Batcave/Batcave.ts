...
import {
    resetSettings
} from './BatcaveSettings'

const BATCAVE_DOMAIN = 'https://batcave.biz'
...
export const BatcaveInfo: SourceInfo = {
    ...
    author: 'Generated',
    authorWebsite: '',
    description: 'Extension that pulls manga from BatCave',
...
export class Batcave implements SearchResultsProviding, MangaProviding, ChapterProviding, HomePageSectionsProviding {
    ...
    async getViewMoreItems(homepageSectionId: string, metadata: any): Promise<PagedResults> {
        ...
        const request = App.createRequest({
            url: `${BATCAVE_DOMAIN}/${homepageSectionId}?page=${page}`,
            method: 'GET'
        })
        const response = await this.requestManager.schedule(request, 1)
        const $ = this.cheerio.load(response.data as string)
    ...
    async getSearchResults(query: SearchRequest, metadata: any): Promise<PagedResults> {
        ...
        const request = App.createRequest({
            url: `${BATCAVE_DOMAIN}/search?q=${searchQuery}&page=${page}`,
            method: 'GET'
        })
        const response = await this.requestManager.schedule(request, 1)
        const $ = this.cheerio.load(response.data as string)
